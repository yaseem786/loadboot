...
**What shipped (staging only — prod on Yaseen's OK):** a broker posts in minutes on a **live FMCSA authority check**; documents move to where they matter.
...
3. **Prod DB, in this order, after reading the migration header:** `bl_bp_0312_broker_trust_tiers.sql` → `bl_bp_0312b_broker_copy.sql` → `bl_bp_0312c_partner_signup_fix.sql` → `bl_bp_0313_broker_identity.sql` (0312a/0312d are staging-only). Say "prod pe chalao" and this session applies them. Then run once: `select app_private.broker_screen_collect();` and confirm cron `lb-broker-screen-collect` exists. Re-check `has_function_privilege('anon', …)` on the five non-token RPCs (the migration aborts if any leaks).
