...
All search volumes are **UNKNOWN**. Obtain Search Console evidence before choosing close variants. “New” means proposed only if the existing inventory lacks equivalent intent. Most work should update existing pages. One canonical URL per intent; preserve URLs and use reviewed redirects only where justified.
