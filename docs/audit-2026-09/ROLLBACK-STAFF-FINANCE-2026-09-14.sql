-- Restore five exact staff-finance bodies; keep shared session helper and other protected endpoints.
DO $rollback$
DECLARE old_acl text;
BEGIN
 IF md5(pg_get_functiondef('public.cc_carrier_payment_profile(uuid)'::regprocedure)) IS DISTINCT FROM '0accf614ee85da6432cc5407f6e7528c' THEN RAISE EXCEPTION 'Rollback source drift: cc_carrier_payment_profile'; END IF;
 IF md5(pg_get_functiondef('public.cc_payment_profiles_queue(text)'::regprocedure)) IS DISTINCT FROM 'df10f596cd366173a6dc461e0dcba6f0' THEN RAISE EXCEPTION 'Rollback source drift: cc_payment_profiles_queue'; END IF;
 IF md5(pg_get_functiondef('public.cc_referral_payout_decide(uuid,text,text)'::regprocedure)) IS DISTINCT FROM '63fd9206e1202df0a6ecca3b5e6ae671' THEN RAISE EXCEPTION 'Rollback source drift: cc_referral_payout_decide'; END IF;
 IF md5(pg_get_functiondef('public.cc_referral_payout_queue(text)'::regprocedure)) IS DISTINCT FROM 'bd426088ee0296fe48ebc4cbf01dcb7e' THEN RAISE EXCEPTION 'Rollback source drift: cc_referral_payout_queue'; END IF;
 IF md5(pg_get_functiondef('public.cc_verify_payment_profile(uuid,boolean,text)'::regprocedure)) IS DISTINCT FROM 'd65ba5a254bb8b1486ef3941084c42eb' THEN RAISE EXCEPTION 'Rollback source drift: cc_verify_payment_profile'; END IF;
...
$source$;
 IF md5(pg_get_functiondef('public.cc_carrier_payment_profile(uuid)'::regprocedure)) IS DISTINCT FROM '6033fb014532f5e0cf92c4a79a527a6e' OR (SELECT proacl::text FROM pg_proc WHERE oid='public.cc_carrier_payment_profile(uuid)'::regprocedure) IS DISTINCT FROM old_acl THEN RAISE EXCEPTION 'Rollback mismatch: cc_carrier_payment_profile'; END IF;
...
$source$;
 IF md5(pg_get_functiondef('public.cc_payment_profiles_queue(text)'::regprocedure)) IS DISTINCT FROM '699ac3566ead0af78000a43b10767a91' OR (SELECT proacl::text FROM pg_proc WHERE oid='public.cc_payment_profiles_queue(text)'::regprocedure) IS DISTINCT FROM old_acl THEN RAISE EXCEPTION 'Rollback mismatch: cc_payment_profiles_queue'; END IF;
...
$source$;
 IF md5(pg_get_functiondef('public.cc_referral_payout_decide(uuid,text,text)'::regprocedure)) IS DISTINCT FROM 'd79403d32cc9aa98867bf6e3d47558f4' OR (SELECT proacl::text FROM pg_proc WHERE oid='public.cc_referral_payout_decide(uuid,text,text)'::regprocedure) IS DISTINCT FROM old_acl THEN RAISE EXCEPTION 'Rollback mismatch: cc_referral_payout_decide'; END IF;
...
$source$;
 IF md5(pg_get_functiondef('public.cc_referral_payout_queue(text)'::regprocedure)) IS DISTINCT FROM 'cbad02dd128029aae1db46a833667d0b' OR (SELECT proacl::text FROM pg_proc WHERE oid='public.cc_referral_payout_queue(text)'::regprocedure) IS DISTINCT FROM old_acl THEN RAISE EXCEPTION 'Rollback mismatch: cc_referral_payout_queue'; END IF;
...
$source$;
 IF md5(pg_get_functiondef('public.cc_verify_payment_profile(uuid,boolean,text)'::regprocedure)) IS DISTINCT FROM 'a7570054ceca4dc83f4d5f71360c7b78' OR (SELECT proacl::text FROM pg_proc WHERE oid='public.cc_verify_payment_profile(uuid,boolean,text)'::regprocedure) IS DISTINCT FROM old_acl THEN RAISE EXCEPTION 'Rollback mismatch: cc_verify_payment_profile'; END IF;
END $rollback$;
