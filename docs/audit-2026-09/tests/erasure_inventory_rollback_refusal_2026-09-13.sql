...
DO $test$
DECLARE uid uuid:=gen_random_uuid(); rid bigint; refused boolean:=false;
BEGIN
 INSERT INTO auth.users(id,email,raw_user_meta_data) VALUES(uid,'audit-inventory-rollback-'||uid||'@example.invalid','{"role":"driver"}');
 INSERT INTO app_private.account_deletion_requests(user_id,email) VALUES(uid,'audit-inventory-rollback-'||uid||'@example.invalid') RETURNING id INTO rid;
 INSERT INTO app_private.account_erasure_inventories(request_id,user_id,items) VALUES(rid,uid,'[]');
 BEGIN EXECUTE $rollback_sql$-- Guarded staging rollback. Refuse to discard any captured inventory.
DO $rollback$
BEGIN
...
$rollback_sql$;
 EXCEPTION WHEN raise_exception THEN
  IF SQLERRM NOT LIKE 'Inventory evidence exists%' THEN RAISE; END IF; refused:=true;
 END;
 ASSERT refused,'Rollback discarded inventory';
 ASSERT EXISTS(SELECT 1 FROM app_private.account_erasure_inventories WHERE request_id=rid),'Inventory lost';
END $test$;
SELECT 'PASS: populated inventory refuses rollback and preserves evidence' result; ROLLBACK;
