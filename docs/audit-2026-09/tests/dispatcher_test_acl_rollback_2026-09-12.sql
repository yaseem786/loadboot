...
SET LOCAL ROLE anon;
DO $before$ DECLARE r record; BEGIN
 FOR r IN SELECT * FROM (VALUES ('cc_dispatcher_test_invite',public.cc_dispatcher_test_invite(null,30,24)),
('cc_dispatcher_test_review',public.cc_dispatcher_test_review(null)),
('cc_dispatcher_test_score',public.cc_dispatcher_test_score(null,'{}',null,null)),
('dispatcher_test_my',public.dispatcher_test_my()),
('dispatcher_test_save',public.dispatcher_test_save(null,'synthetic',0,0)),
('dispatcher_test_start',public.dispatcher_test_start()),
('dispatcher_test_submit',public.dispatcher_test_submit('{}'))) v(name,result) LOOP
  ASSERT r.result->>'error' IN ('not authorized','not signed in'), 'Existing guard did not refuse anonymous caller';
 END LOOP;
END $before$;
RESET ROLE;
...
END $allowed$;
RESET ROLE;
SELECT 'PASS: seven existing anon guards refuse; seven ACLs narrowed; bodies and auth/service grants preserved; exact surface diff; authenticated empty-state and nonstaff checks; transaction rolled back' result;
ROLLBACK;
