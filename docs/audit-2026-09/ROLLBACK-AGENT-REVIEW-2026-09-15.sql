...
BEGIN
 ...
 ('public.cc_agent_payout_verify(uuid,boolean,text)',ARRAY['4f29c965c80615fc45b7941b2fc3440a','fdcfb71d654ea07e77d223df997ffb7b'])
  ...
  IF item.signature='public.cc_agent_payout_verify(uuid,boolean,text)' THEN
   ...
   patched:=replace(patched,E'  if p_ok is null then raise exception ''an explicit approve or reject decision is required'' using errcode=''22023''; end if;\n  if not p_ok and','  if not p_ok and');
  END IF;
  IF NOT md5(patched)=ANY(item.hashes) OR patched=original THEN RAISE EXCEPTION 'Rollback source drift: %',item.signature; END IF;
  EXECUTE patched;
