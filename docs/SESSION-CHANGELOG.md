...
## 2026-07-28 — Preferences Completion System + Booking-ping + anon-surface restore (cloud session)
- **SECURITY — anon surface restore (bl_sec_0190, BOTH DBs):** drift found (staging 143 / prod 89 anon-executable
  SECURITY DEFINER fns, direct grants incl. staff cc_*). Restored to a 26-fn deliberate allowlist (original 5 +
  all_flags + get_public_market_rates + lc_* chat/concierge + lb_email_claim_* + lb_email_load_ingest/reply_merge +
  lb_email_ping_* + retell_webhook + eld_ingest + outreach_unsubscribe). **New check number after every migration: 26** (not 5).
- **Carrier Preferences Completion backend (bl_pref_0188/0189, BOTH DBs, parity):** carrier_dispatch_prefs +
  operating_radius_miles/home_time/load_size/equipment_detail/min_total_rate/facility_likes/facility_dislikes/
  external_boards/prefs_sections; RPCs cc_prefs_profile_strength + cc_prefs_save_section (skip=3-day snooze);
  cc_match_rank got a prefs_fit factor (lane 7 / rate-floor 4 / weekend 2 + risks). Staging matrix PASS.
- **Preferences frontend:** NEW app/carrier/prefs-strength.js (Profile Strength ring card on dashboard +
  one-section-at-a-time micro-ask popups, chips UI, +N% reward framing, 6h nudge throttle); api.js wrappers;
  app.js wiring (prefsHost after setupCard + maybeShowMicroAsk). CC: carrier360 prefs card shows all new fields +
  "has DAT/Truckstop" banner; dispatchers SOP editor auto-prefills lanes/rate/equipment/home-time from carrier
  prefs + boards hint chip.
- **Booking-ping (bl_ping_0191/0192 BOTH DBs + load-mail v7 BOTH + broker-ping.html):** carrier book request on an
  email-sourced load → 4h hold + broker address-confirm email (link + reply-path both work; declines handled) →
  confirm books load, creates trip, cancels competitors, notifies carrier. E2E staging 12/12 PASS.
  cc_request_book_load now returns the real hold window. broker-ping.html is generated by build_site.py
  (noindex + sitemap-excluded). bl_disp_0187 applied to staging (parity).
- Gates: BUILD OK, ESM 122 files PASS. check_imports has a PRE-EXISTING false positive
  (partnerIntake.js ccAskReschedule via dynamic import).
- REMAINING: owner local visual test + commit/push; FULL SITE+PORTALS AUDIT (register started in docs/AUDIT-2026-07-28.md).
