-- PROD APPLY — bl_dmail_0364 (withdrawal e-mails + in-app notifications + identity reset on release)
-- STATUS: APPLIED TO PROD 20 Sep 2026 by Claude (one merged transaction, 0364 + 0364a final state).
-- Kept as the replay record. Includes bl_dmail_0364a (line-replacement notice) below.
begin;
...
-- ---------------------------------------------------------------- verify (must be: n=33, new_fns=5, reset_ok=t)
select
  (select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace
     where n.nspname='public' and p.prosecdef and has_function_privilege('anon',p.oid,'execute')) as anon_secdef_n,
  (select count(*) from pg_proc p join pg_namespace n on n.oid=p.pronamespace
     where n.nspname='app_private' and p.proname in
       ('dmail_signature_neutral','dmail_identity_reset','dmail_withdraw_html','dmail_withdraw_email','disp_phone_line_withdrawn_email')) as new_fns,
  (select count(*)=0 from pg_proc p join pg_namespace n on n.oid=p.pronamespace
     where n.nspname='app_private' and p.proname in
       ('dmail_esc','dmail_li','dmail_welcome_html','dmail_signature_html','dmail_signature_neutral','dmail_identity_reset','dmail_withdraw_html','dmail_withdraw_email','disp_phone_line_withdrawn_email')
       and has_function_privilege('public',p.oid,'execute')) as no_public_grants,
  (select prosrc like '%dmail_identity_reset%' from pg_proc p join pg_namespace n on n.oid=p.pronamespace
     where n.nspname='public' and p.proname='cc_dmail_assign') as assign_patched,
  (select prosrc like '%disp_phone_line_withdrawn_email%' from pg_proc p join pg_namespace n on n.oid=p.pronamespace
     where n.nspname='public' and p.proname='cc_dialer_line_release') as release_patched,
  (select prosrc like '%line_changed%' from pg_proc p join pg_namespace n on n.oid=p.pronamespace
     where n.nspname='public' and p.proname='cc_dialer_line_upsert') as upsert_patched;
