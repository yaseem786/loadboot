-- PROD APPLY — paste whole file in Supabase SQL Editor (project rwscphuhpjoudvljvmdk). Expected: "Success. No rows returned". Choose "Run and enable RLS" if asked (the file already enables it). Additive; safe to re-run.
