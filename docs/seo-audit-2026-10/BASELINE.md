...
| S6 | Weakly linked pages (≤ 2 in-content inbound) | 14 — incl. all three state dispatcher pages (TX/GA/CA: 1–2 links each, all from blog.html), otr/regional/local dispatch (only each other), full-truckload-vs-ltl, should-i-buy-a-truck-before-2027 | Internal-link pass in the page sessions of their hub pages — **fixed 2026-09-25** (Related-grid link pass, LEDGER S6 row): all 11 content pages now ≥ 3 in-content inbound, incl. two new orphans (ai-dispatch-for-owner-operators, truck-dispatcher-vs-dispatch-software). Left: 7 utility / chrome-linked pages |
...
| S8 | Thin pages (< 300 words) | status 215, accessibility 269 (referral gone) | Expected — **closed 2026-09-25, no action**: both are utility pages with nothing to rank for; padding them to 300 words would be filler |
| S9 | `sitemap.xml` carries `<changefreq>` only | all | Google ignores changefreq; `<lastmod>` is the field it reads. Only worth adding if it is a real per-page date (build has none) — **do not fake it with the build date** — **fixed 2026-09-25**: `build_site.py` sitemap block reads each page's own JSON-LD `dateModified` (article first-commit dates, market-report `as_of`, policy dates) → 50 of 135 URLs get `<lastmod>`, the other 85 get none. No build date anywhere |
| S10 | `unsub.html` is excluded from the sitemap but not `noindex`; Bing sends it 32 organic landings in 90 d, `privacy.html` 15 | 2 | Add `<meta name="robots" content="noindex">` to unsub.html; privacy is fine — **closed 2026-09-24** via `X-Robots-Tag: noindex, nofollow` on `/unsub.html` in `_headers` (§2); `/unsub` 301s to it, so no meta tag needed |
...
| S12 | Rendered pass: every page logs 3 console errors — clarity.ms and googletagmanager blocked by the container proxy, trustpilot widget 405 over http from localhost | harness artefacts, not site bugs | none — closed |
