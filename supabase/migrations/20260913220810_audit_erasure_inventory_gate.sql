...
BEGIN
 IF md5(pg_get_functiondef('public.cc_account_deletion_process(bigint,text,text)'::regprocedure)) IS DISTINCT FROM '1b064d8662a4b4c2e9b3bc2cdb684f7f' THEN RAISE EXCEPTION 'Deletion source drift'; END IF;
 SELECT proacl::text INTO old_acl FROM pg_proc WHERE oid='public.cc_account_deletion_process(bigint,text,text)'::regprocedure;
...
$source$;
 IF (SELECT proacl::text FROM pg_proc WHERE oid='public.cc_account_deletion_process(bigint,text,text)'::regprocedure) IS DISTINCT FROM old_acl THEN RAISE EXCEPTION 'Deletion grants changed'; END IF;
END $patch$;
