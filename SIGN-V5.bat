@echo off
REM ============================================================
REM  SIGN-V5.bat - Android app v1.0.4 (versionCode 5) ko upload
REM  key se sign karta hai. Keystore aur password aap ke computer
REM  se bahar nahi jate. Output: release\loadboot-v5.aab
...
echo ==================================================
echo   LoadBoot - app v1.0.4 sign
echo ==================================================
echo Folder: "%CD%"
echo.

REM ---- 1. files check -------------------------------------------------
if not exist "release\loadboot-v5-unsigned.aab" goto :no_aab
...
copy /y "release\loadboot-v5-unsigned.aab" "release\loadboot-v5.aab" >nul
echo.
if not defined SP goto :ask_pw
echo [OK] Password PLAY-KEYSTORE-NOTE.txt se le liya. Sign ho raha hai...
"%JBIN%\jarsigner.exe" -keystore loadboot-upload.keystore -storepass:env SP -digestalg SHA-256 -sigalg SHA256withRSA "release\loadboot-v5.aab" loadboot >nul
goto :signed
:ask_pw
echo Keystore ka password type karein - type karte waqt dikhega nahi - phir Enter:
"%JBIN%\jarsigner.exe" -keystore loadboot-upload.keystore -digestalg SHA-256 -sigalg SHA256withRSA "release\loadboot-v5.aab" loadboot
:signed
if errorlevel 1 goto :sign_failed
set "SP="

REM ---- 4. verify: sahi key se sign hua? -------------------------------
"%JBIN%\keytool.exe" -printcert -jarfile "release\loadboot-v5.aab" | findstr /c:"%FP%" >nul
if errorlevel 1 goto :wrong_key
echo.
echo ==================================================
echo   [OK] TAYYAR: release\loadboot-v5.aab
echo   Upload key sahi hai: A7:F2 ... F1:48
echo   Ab yeh file Play Console par upload karein.
echo ==================================================
explorer /select,"%CD%\release\loadboot-v5.aab"
goto :end

:no_aab
echo [X] release\loadboot-v5-unsigned.aab nahi mili.
...
del "release\loadboot-v5.aab" >nul 2>&1
goto :end
:wrong_key
echo.
