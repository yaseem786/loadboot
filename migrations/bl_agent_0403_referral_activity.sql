-- bl_agent_0403 — per-referral activity timeline for the Referral Partner home (22 Sep 2026).
-- Yaseen: "don't track just one load — show the partner every activity of their referral that
-- matters to them". One authenticated RPC, read-only, scoped to the caller's own referral edges.
-- Exposes only what a partner needs: milestones + lanes + their own commission money.
-- Never: rates, broker names, driver/VIN/phone, documents' contents. Additive; nothing else touched.
