-- Staging parity for two existing production agent payout-review RPCs.
-- Definitions and grants captured 2026-09-09. No payment or sender invoked by this migration.
DO $guard$ BEGIN
 IF to_regprocedure('public.cc_agent_payout_approve_method(uuid,text)') IS NOT NULL THEN
  RAISE EXCEPTION 'cc_agent_payout_approve_method already exists; re-sync before applying parity migration';
 END IF;
END $guard$;
CREATE OR REPLACE FUNCTION public.cc_agent_payout_approve_method(p_user uuid, p_note text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION public.cc_agent_payout_approve_method(uuid,text) TO authenticated,service_role;

DO $guard$ BEGIN
 IF to_regprocedure('public.cc_agent_payout_request_details(uuid,text[],text)') IS NOT NULL THEN
  RAISE EXCEPTION 'cc_agent_payout_request_details already exists; re-sync before applying parity migration';
 END IF;
END $guard$;
CREATE OR REPLACE FUNCTION public.cc_agent_payout_request_details(p_user uuid, p_fields text[], p_note text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'app_private, public'
...
GRANT EXECUTE ON FUNCTION public.cc_agent_payout_request_details(uuid,text[],text) TO authenticated,service_role;
