-- ct-waveBK · Account Health v2.1 — grouped factor model (Amazon Seller Central style)
...
begin
  ...
  if v_kind = 'carrier' then
    ...
    -- v2.1: submitted-but-pending docs are IN REVIEW — zero penalty.
    -- Only truly missing/rejected/expired mandatory docs deduct.
    ------------------------------------------------------------------
    items := '[]'::jsonb; gd := 0;
    begin
      select count(*) filter (where coalesce(c.status,'') <> 'pending'
                                and not (coalesce(c.status,'') = 'valid' and (c.expiry_date is null or c.expiry_date >= current_date))),
             count(*) filter (where coalesce(c.status,'') = 'pending')
        into n1, n2
        ...
        where r.active and r.mandatory
          and (r.condition_key is null
               or (r.condition_key = 'hazmat' and app_private.carrier_is_hazmat(v_org))
               or (r.condition_key = 'bank' and app_private.carrier_has_bank(v_org)));
    exception when others then n1 := 0; n2 := 0; end;
    d := case when n1 > 0 then 12 else 0 end;
    items := items || jsonb_build_object('key','mandatory','label','Mandatory documents',
      'value', case when n1 > 0 then n1 || ' missing' when n2 > 0 then 'in review' else 'all verified' end,
      'state', case when n1 > 0 then 'missing' when n2 > 0 then 'review' else 'ok' end,
      'target','all submitted','deducted',d,
      'basis', case when n1 > 0 then n1 || ' required document(s) not submitted yet' || case when n2 > 0 then ' - ' || n2 || ' already submitted are under review (no penalty)' else '' end
                    when n2 > 0 then n2 || ' document(s) submitted and under LoadBoot review - submitted documents never count against you'
                    else 'All required documents are verified and current' end,
      'improve', case when n1 > 0 then 'Upload the missing documents from your Documents page - the deduction clears the moment you submit them.'
                      when n2 > 0 then 'Nothing to do - our team is reviewing. You keep full points while review is in progress.'
                      else 'Keep documents current and replace them before expiry.' end);
    gd := gd + d;
    select count(*) filter (where expiry_date < current_date),
      ...
      from app_private.carrier_compliance where carrier_id = v_org and status = 'valid' and expiry_date is not null;
