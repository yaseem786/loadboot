-- bl_wa_0379 - the 15-second double-send guard now covers attachments too.
-- bl_wa_0371 stopped the same LINE going out twice on a double click; bl_wa_0375 added files and the guard
-- skipped them, because it compares bodies and a voice note has no body. A dispatcher who taps Send twice, or
-- whose browser retries, sent the same photo twice. Identity for a file is the uploaded object path, which the
-- browser makes fresh (crypto.randomUUID) for every upload - so a genuine second file is never refused.
-- Everything else in wa_send_prepare is bl_wa_0375, unchanged. APPLY ON STAGING FIRST.
...
declare v_uid uuid := auth.uid(); v_role text; cfg app_private.dialer_config; t app_private.wa_threads;
  ...
  md jsonb := case when jsonb_typeof(p->'media') = 'object' then p->'media' else null end;
  v_mkind text; v_mime text; v_path text; v_fname text; v_media jsonb;
begin
  ...
  if md is not null then
    -- ---------------- an attachment (bl_wa_0375)
    v_path  := nullif(btrim(coalesce(md->>'path','')),'');
    v_mime  := lower(coalesce(nullif(md->>'mime',''), 'application/octet-stream'));
    v_fname := nullif(btrim(coalesce(md->>'file_name','')),'');
    v_mkind := case
        when md->>'kind' in ('image','audio','video','document') then md->>'kind'
        when v_mime like 'image/%' then 'image'
        when v_mime like 'audio/%' then 'audio'
        when v_mime like 'video/%' then 'video'
        else 'document' end;
    if v_path is null then return jsonb_build_object('ok', false, 'error','That file was not uploaded.'); end if;
    if not v_open then
      return jsonb_build_object('ok', false, 'error','The 24-hour window is closed - an attachment cannot be sent, only an approved template.', 'needs_template', true); end if;
    v_body := btrim(coalesce(md->>'caption', p->>'body', ''));
    if length(v_body) > 1000 then return jsonb_build_object('ok', false, 'error','That caption is too long (1000 characters max).'); end if;
    v_kind := 'media';
    -- stored in the same shape inbound media uses, so one reader serves both
    v_media := jsonb_build_object('type', v_mkind, v_mkind,
      jsonb_strip_nulls(jsonb_build_object('path', v_path, 'mime_type', v_mime, 'filename', v_fname,
        'caption', nullif(v_body,''), 'voice', case when coalesce((md->>'voice')::boolean, false) then true else null end)));
    v_payload := jsonb_build_object('type', v_mkind, v_mkind,
      jsonb_strip_nulls(jsonb_build_object('caption', case when v_mkind in ('image','video','document') then nullif(v_body,'') end,
                                           'filename', case when v_mkind = 'document' then v_fname end)));
  elsif nullif(p->'template'->>'name','') is not null then
  ...
  -- bl_wa_0379: the same guard for an attachment. A file cannot be compared by its body (a voice note has
  -- none), so the uploaded object path is the identity: the same upload twice inside 15 seconds is a double
  -- click, never a decision. Two different files get two different random paths, so nothing real is blocked.
  if md is not null and exists (select 1 from app_private.wa_messages x
              where x.thread_id = t.id and x.direction = 'outbound'
                and x.media->(x.media->>'type')->>'path' = v_path
                and x.created_at > now() - interval '15 seconds') then
    return jsonb_build_object('ok', false, 'error','That file just went out a moment ago.');
  end if;
  if md is null and exists (select 1 from app_private.wa_messages x
  ...
  insert into app_private.wa_messages (thread_id, direction, sender_user_id, kind, body, media, template_name, template_vars, status)
  values (t.id, 'outbound', v_uid, v_kind, v_body, v_media, tpl.name, v_vars, 'queued') returning * into m;
  update app_private.wa_threads set last_at = now(),
     last_body = left(coalesce(nullif(v_body,''), '[' || coalesce(v_mkind,'message') || ']'), 300),
     last_direction = 'outbound', last_outbound_at = now(), updated_at = now() where id = t.id;

  return jsonb_build_object('ok', true, 'id', m.id, 'thread_id', t.id, 'from', cfg.wa_number, 'to', t.counterparty,
    'whatsapp_message', v_payload, 'media_path', v_path, 'media_kind', v_mkind,
    'messaging_profile_id', cfg.wa_messaging_profile_id, 'message', app_private.wa_msg_json(m));
end $$;
