-- bl_wa_0375 — sending a photo, a document or a voice note, not just words.
-- The browser uploads the file to the PRIVATE wa-media bucket (bl_wa_0374) and passes only its path here;
-- Postgres records the message and hands the edge function the path, which turns it into a short-lived signed
-- link for Telnyx to fetch. The file is never public and the browser never builds the link itself.
-- Meta's rules still apply and are still enforced here: media is a free-form message, so it may only go out while
-- the 24-hour window is open. A template cannot carry one of these.
-- Which mime types Meta accepts is NOT re-implemented here (their list changes); an unsupported file comes back
-- as a Telnyx error on the bubble rather than a silent failure.
-- Everything else is bl_wa_0371's wa_send_prepare, unchanged. APPLIED ON STAGING 21 Sep 2026.
