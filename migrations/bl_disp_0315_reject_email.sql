-- bl_disp_0315 — a proper rejection e-mail for dispatcher candidates (14 Sep 2026).
-- Before: every status went through disp_notify, so a rejected candidate received subject
-- "LoadBoot: Application closed", a bare <h2> + the staff note, an "open your dashboard" button and the
-- footer line "this is a transactional notice for an active assignment" — wrong on all three counts.
-- Now: rejected has its own branded e-mail (greeting, the staff note as the reason, a what-this-means
-- box, a careers CTA, recruiting signature). The in-app notification is unchanged; only the e-mail moved.
-- The staff note typed in the CC reject box is still what the candidate reads — that has not changed.
-- Applied to staging and production 14 Sep 2026. Anon SECURITY DEFINER surface unchanged (33).
...
declare v_mail text; v_name text; v_reason text; v_html text; v_text text;
begin
...
begin
  ...
  if position('disp_reject_email' in src) > 0 then return; end if;
  anchor := '''/app/agent/#dashboard'', v_new in (''trial'',''verified'',''active'',''rejected'',''suspended''));';
  if position(anchor in src) = 0 then raise exception 'bl_disp_0315 anchor missing'; end if;
  src := replace(src, anchor,
    '''/app/agent/#dashboard'', v_new in (''trial'',''verified'',''active'',''suspended''));
