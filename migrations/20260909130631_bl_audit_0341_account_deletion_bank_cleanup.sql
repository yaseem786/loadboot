-- STAGING ONLY; follow-up to applied 0339. No account is processed by this migration.
-- Empty bank values respect existing NOT NULL columns; approval is invalidated.
DO $guard$
BEGIN
 IF pg_get_functiondef('public.cc_account_deletion_process(bigint,text,text)'::regprocedure) IS DISTINCT FROM $expected$CREATE OR REPLACE FUNCTION public.cc_account_deletion_process(p_id bigint, p_action text, p_note text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
$expected$ THEN
  RAISE EXCEPTION 'Deletion processor changed; re-sync before applying bank fix';
 END IF;
END $guard$;
