...
declare
  ...
  v_wa text := coalesce((select nullif(btrim(value),'') from app_private.disp_desk_config where key = 'whatsapp_number'), (select nullif(btrim(wa_number),'') from app_private.dialer_config order by updated_at desc nulls last limit 1));   -- company WhatsApp = the live Telnyx WA number unless overridden
