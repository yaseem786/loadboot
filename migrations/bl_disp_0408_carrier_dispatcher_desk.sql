...
-- Additive + reversible. No existing function is modified.
...
declare
  ...
  v_wa text := (select nullif(btrim(value),'') from app_private.disp_desk_config where key = 'whatsapp_number');
...
begin
  ...
  v_ready := (
    with docs as (select type, status from public.documents where carrier_id = v_org),
  ...
  return jsonb_build_object(
    ...
    'program', jsonb_build_object('fee_pct', 5, 'escalation_email', v_esc, 'whatsapp_configured', v_wa is not null)
  );
end $$;
