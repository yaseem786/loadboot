...
import { renderAvailabilityCard, goFleet, expiryLine, fleetGateBody, buildPlacePicker, buildDestPicker, AVAIL_RULE } from './availability.js';
...
/* ---- Lane-rate context at the negotiation moment (2026-08 audit: DAT pattern) ---- */
function _lbSt(x) { const m = String(x || '').match(/,\s*([A-Za-z]{2})(?:\s|$)/); return m ? m[1].toUpperCase() : null; }
function lbLaneRateCard(l) {
  const host = h('div');
  (async () => {
    try {
      const o = _lbSt(l.origin), d = _lbSt(l.destination);
      if (!o || !d) return;
      const r = await laneRate(o, d, l.equipment || null, l.miles || null);
      if (!r || (!r.avg_rpm && !r.avg && !r.low)) return;
      const f = (v) => (v == null ? null : '$' + Number(v).toFixed(2));
      const low = f(r.low_rpm ?? r.low), avg = f(r.avg_rpm ?? r.avg), high = f(r.high_rpm ?? r.high);
      const myRpm = (l.rate && Number(l.miles) > 0) ? Number(l.rate) / Number(l.miles) : null;
      const rows = [low ? 'low ' + low : null, avg ? 'avg ' + avg : null, high ? 'high ' + high : null].filter(Boolean).join(' · ');
      if (!rows) return;
      host.appendChild(h('div', { style: 'background:rgba(8,131,247,.08);border:1px solid rgba(8,131,247,.3);border-radius:12px;padding:10px 13px;font-size:.8rem;margin-bottom:12px' }, [
        h('b', { style: 'display:block;margin-bottom:2px' }, '📊 ' + o + ' → ' + d + ' market ($/mi): ' + rows),
        myRpm && (r.avg_rpm ?? r.avg) ? h('span', { style: 'color:' + (myRpm >= Number(r.avg_rpm ?? r.avg) ? '#16a34a' : '#d97706') + ';font-weight:700' },
          'This load pays $' + myRpm.toFixed(2) + '/mi — ' + (myRpm >= Number(r.avg_rpm ?? r.avg) ? 'at or above' : 'below') + ' the lane average.') : null,
      ].filter(Boolean)));
    } catch (_) {}
  })();
  return host;
}
/* ---- Store-review ask (Play In-App-Review has no TWA web API — deep-link the listing) ---- */
function lbMaybeAskStoreReview() {
  try {
    if (Date.now() - Number(localStorage.getItem('lb_store_rev') || 0) < 90 * 864e5) return;
    localStorage.setItem('lb_store_rev', String(Date.now()));
    const isAndroid = /android/i.test(navigator.userAgent);
    const card = h('div', { style: 'position:fixed;left:12px;right:12px;bottom:calc(74px + env(safe-area-inset-bottom));z-index:9998;background:#10223B;color:#eaf1fb;border:1px solid rgba(255,255,255,.14);border-radius:16px;padding:14px 16px;box-shadow:0 18px 44px -12px rgba(0,0,0,.65);max-width:480px;margin:0 auto' }, [
      h('div', { style: 'display:flex;align-items:center;gap:8px' }, [h('b', { style: 'font-size:14px' }, '⭐ Enjoying LoadBoot?'), h('button', { style: 'margin-left:auto;background:none;border:0;color:#8ea2c3;font-size:18px;cursor:pointer', onClick: () => card.remove() }, '×')]),
      h('div', { style: 'font-size:.84rem;color:#b9c6db;margin-top:4px' }, 'A quick rating helps more carriers find verified freight.'),
      isAndroid ? h('a', { class: 'cp-btn cp-btn-sm', style: 'margin-top:10px;display:inline-block;text-decoration:none', href: 'https://play.google.com/store/apps/details?id=com.loadboot.app', target: '_blank', rel: 'noopener' }, 'Rate on Google Play') : null,
    ].filter(Boolean));
    document.body.appendChild(card);
    setTimeout(() => { try { card.remove(); } catch (_) {} }, 25000);
  } catch (_) {}
}
/* ---- 2026-08 audit: blocking alert() → premium toast (57 call sites upgraded at once).
   Module-scope shadow: every alert(...) in this file now renders the branded toast.
   Verified: no call site depends on alert's blocking behaviour or return value. ---- */
function alert(msg) { try { lbToast(String(msg == null ? 'Something went wrong.' : msg)); } catch (_) { try { window.alert(msg); } catch (__) {} } }
/* ---- Saved loads (favorites) — 2026-08 audit: DAT/Truckstop table-stakes ---- */
function _lbFavs() { try { return new Set(JSON.parse(localStorage.getItem('lb_fav_loads') || '[]')); } catch (_) { return new Set(); } }
function lbFavToggle(id) { const s9 = _lbFavs(); const k9 = String(id); const on9 = !s9.has(k9); if (on9) s9.add(k9); else s9.delete(k9); try { localStorage.setItem('lb_fav_loads', JSON.stringify(Array.from(s9).slice(-200))); } catch (_) {} return on9; }
function lbToast(msg, tone, title) {
  const t = toneOf(tone || 'urgent');
  try { haptic(tone === 'success' ? 'success' : tone === 'action' ? 'tap' : 'warn'); } catch (_) {}
...
function openModal(title, children) {
  ...
  card.setAttribute('role', 'dialog'); card.setAttribute('aria-modal', 'true'); card.setAttribute('aria-label', title || 'Dialog');
  // Focus trap (2026-08 audit): Tab cycles inside the dialog instead of escaping behind the scrim.
  const onTrap = (e) => {
    if (e.key !== 'Tab') return;
    const els = card.querySelectorAll('a[href],button:not([disabled]),input:not([disabled]),select:not([disabled]),textarea:not([disabled]),[tabindex]:not([tabindex="-1"])');
...
const ic = (name) => ({
  // bl_ui_0394 — five keys (support, user, idcard, pin, logout) were declared TWICE in this
  // map with different paths. JS keeps the LAST one, so the first of each pair was dead code
  // that silently swallowed any edit made to it. The dead halves are gone; every glyph on
  // screen is unchanged, because the surviving definition is the one that was already drawing.
  ...
  docs: 'M6 2h9l5 5v15H6zM14 2v6h6', bell: 'M18 8a6 6 0 00-12 0c0 7-3 9-3 9h18s-3-2-3-9M13.7 21a2 2 0 01-3.4 0', shield: 'M12 2l8 3v6c0 5-3.4 8.4-8 11-4.6-2.6-8-6-8-11V5z',
  menu: 'M3 6h18M3 12h18M3 18h18',
  sos: 'M10.3 3.9L1.8 18a2 2 0 001.7 3h17a2 2 0 001.7-3L13.7 3.9a2 2 0 00-3.4 0zM12 9v4M12 17h.01',
  cog: 'M12 15a3 3 0 100-6 3 3 0 000 6zM12 2v3M12 19v3M4.9 4.9l2.1 2.1M17 17l2.1 2.1M2 12h3M19 12h3M4.9 19.1L7 17M17 7l2.1-2.1', inbox: 'M22 12h-6l-2 3h-4l-2-3H2M5.5 6h13l3.5 6M2 12l3.5-6',
...
function authScreen() {
  ...
  // 10DLC / TCR: SMS consent is a SEPARATE, OPTIONAL, UNCHECKED box. The mobile number itself is used for
  // voice dispatch and is required; texting only ever happens if the carrier ticks this box themselves.
  const smsOk = h('input', { type: 'checkbox', style: 'margin:2px 0 0;flex:none;width:16px;height:16px;accent-color:#0883F7' });
  const extra = h('div', { style: 'display:none' }, [h('label', { class: 'cp-lbl' }, window.__LB_AGENT ? 'Agency / company (optional)' : 'Company'), company, h('label', { class: 'cp-lbl' }, 'Your name'), name, h('label', { class: 'cp-lbl' }, 'Mobile number'), h('div', { style: 'display:flex;gap:8px' }, [ccSel, phone]),
    // 10DLC opt-in: an OPTIONAL, unchecked checkbox sits next to the disclosure (Telnyx/TCR requirement).
    // Leaving it unticked does not block account creation and we never text that number.
    h('label', { style: 'display:flex;gap:9px;align-items:flex-start;margin:9px 0 0;cursor:pointer' }, [smsOk,
      h('span', { style: 'font-size:11.5px;line-height:1.45;opacity:.8' }, ['Optional \u2014 tick to agree to receive text messages from LoadBoot at the mobile number above about your loads and account (dispatch updates, check calls, paperwork requests). Consent is not a condition of creating an account or of purchasing anything. Message frequency varies. Msg & data rates may apply. Reply STOP to opt out, HELP for help. See our ', h('a', { href: '/terms.html#s11', target: '_blank', rel: 'noopener' }, 'Terms'), ' and ', h('a', { href: '/privacy.html#sms', target: '_blank', rel: 'noopener' }, 'Privacy Policy'), '.'])])]);
  const err = h('div', { class: 'cp-err' });
    ...
    try {
      if (signup) {
        ...
        try {
          ...
          if (smsOk.checked && phv && phv.replace(/\D/g, '').length >= 7) {
...
async function agentPortal(user) {
  ...
  const inviteModal9 = () => {
    ...
    const close = openModal('✉ Send a premium invite — with YOUR link inside', [
      ...
      } }, [icon('clipboard',15),' Copy as message instead (WhatsApp/SMS)']),
    ]);
  };
  const linkCard = () => agCard('🔗 Your referral link', [
    ...
    h('div', { style: 'display:flex;gap:8px;flex-wrap:wrap' }, [
      h('button', { class: 'cp-btn cp-btn-sm', style: 'background:#FC5305', onClick: inviteModal9 }, [icon('mail',15),' Invite by email']),
  ...
  const loadRows = () => {
    ...
    return loads.map((x) => h('div', { class: 'cp-row', style: 'flex-wrap:wrap' }, [
      ...
      x.delivered_at ? h('span', { class: 'cp-pill', style: 'background:rgba(34,197,94,.15);color:#4ade80' }, '✓ DELIVERED')
        : x.trip_status ? h('span', { class: 'cp-pill', style: 'background:rgba(8,131,247,.15);color:#3b9dff' }, [icon('truck',15),' BOOKED'])
  ...
  async function renderDispatcherHome(host) {
    ...
    if (!prof || prof.status === 'applied') {
      ...
      const f = {
        ...
        can_source: sel([['', 'How do you get load-board access today? *'], ['own_paid', 'My OWN paid subscription, in my name (DAT / Truckstop / 123Loadboard…)'], ['employer', 'Through an employer’s or a carrier’s login — I can log in and book myself'], ['none_willing', 'None right now — I will buy my own subscription before a trial starts'], ['none', 'None — I would need LoadBoot to provide access'], ['learning', 'I have not sourced loads yet — still learning']]),
      ...
      const ownWrap = h('div', { hidden: true }, [h('div', { class: 'cp-row-s', style: 'margin:6px 0 2px' }, 'Which board(s) can you log into and book on? *'), ownBoards.box]);
      ...
      const boardWarn = h('div', { hidden: true, role: 'note', style: 'margin-top:8px;padding:10px 12px;border-radius:10px;border:1px solid rgba(252,83,5,.5);background:rgba(252,83,5,.08);font-size:.85rem;line-height:1.6;color:#fdba74' }, [
        h('div', null, 'This role requires working load-board access from week one and loads you find and book yourself. The subscription does not have to be in your name — an employer’s or a carrier’s login is fine — but you must be able to log in today. Applications with no board access are not being moved forward right now. If that changes, pick one of the first two options — otherwise you can still submit and we will keep your details.'),
      ...
      // Owner rule 21 Sep 2026: own_paid AND employer both pass — the subscription need not be in
      // the candidate's own name. none_willing / none / learning are the gap.
      const boardOk9 = (v9) => v9 === 'own_paid' || v9 === 'employer';
    ...
    if (tab === 'dashboard') {
      ...
      const verifyCard =
        ...
        : obStatus === 'info_needed' ? h('div', { style: 'background:rgba(245,158,11,.12);border:1px solid rgba(245,158,11,.45);border-radius:14px;padding:14px 16px;margin-bottom:12px' }, [
            h('div', { style: 'font-weight:800;color:#fbbf24' }, [icon('alert',15),' Verification — more info needed']),
      ...
      if (W.step === 0) {
        ...
        body9 = h('div', null, [
          ...
          d.tax_form === 'w9' ? h('div', { style: 'margin-top:12px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.12);border-radius:12px;padding:12px 14px' }, [
            h('div', { style: 'font-weight:800;font-size:.88rem' }, [icon('receipt',15),' W-9 (substitute) — US person']),
          ...
          d.tax_form === 'w8ben' ? h('div', { style: 'margin-top:12px;background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.12);border-radius:12px;padding:12px 14px' }, [
            h('div', { style: 'font-weight:800;font-size:.88rem' }, [icon('receipt',15),' W-8BEN (substitute) — non-US person']),
            h('div', { style: 'display:flex;gap:12px;flex-wrap:wrap;margin-top:8px' }, [
        ...
        return h('div', { style: 'border:1px solid rgba(255,255,255,.1);border-radius:13px;padding:12px 14px;margin-top:8px;background:rgba(255,255,255,.03)' }, [
          h('div', { style: 'display:flex;gap:10px;flex-wrap:wrap;align-items:center' }, [
            ...
            st9 === 'delivered' ? h('span', { class: 'cp-pill', style: 'background:rgba(34,197,94,.15);color:#4ade80;font-weight:800' }, '✓ DELIVERED')
              : st9 === 'booked' ? h('span', { class: 'cp-pill', style: 'background:rgba(8,131,247,.15);color:#3b9dff;font-weight:800' }, [icon('truck',15),' MOVING'])
      ...
      try {
        ...
        mount(hostE, [h('div', { class: 'cp-cardhead' }, [h('h3', null, [icon('finance',15),' Commission ledger'])]),
        ...
        mount(hostP, h('div', null, [
          ...
          pc.eligible
            ? h('div', { style: 'background:rgba(34,197,94,.1);border:1.5px solid rgba(34,197,94,.45);border-radius:14px;padding:16px 18px;margin-bottom:12px;text-align:center' }, [
                h('div', { style: 'font-weight:900;font-size:1.05rem;color:#4ade80;margin-bottom:10px' }, [icon('check',15),' You are ELIGIBLE — money is ready to move']), reqBtn,
          ...
          pc.bank ? agCard('🏦 Payout account on file', [
            ...
            h('div', { class: 'cp-row-s', style: 'margin-top:6px' }, [icon('lock',15),' Payouts only ever go to this verified account — nobody can redirect your money.']),
          ].filter(Boolean)) : agCard('🏦 Payout account', [h('div', { class: 'cp-row-s' }, 'No account on file yet — add it in Get Verified step 3.'), h('button', { class: 'cp-btn cp-btn-sm', style: 'margin-top:8px', onClick: () => go('verify') }, [icon('shield',15),' Add payout account →'])]),
      ...
      mount(content, h('div', null, [
        ...
        agCard('📄 My signed documents', [
          h('div', { style: 'display:flex;gap:8px;flex-wrap:wrap' }, [
            ...
            h('button', { class: 'cp-btn-ghost', onClick: rulesModal9 }, [icon('docs',15),' Program rules & policies']),
          ]),
        ]),
        agCard('🔒 Security', [
  ...
  const agBellPanel = h('div', { hidden: true, style: 'position:absolute;top:44px;right:0;width:min(360px,86vw);background:#0f1b30;border:1px solid rgba(255,255,255,.14);border-radius:14px;box-shadow:0 18px 50px rgba(0,0,0,.55);z-index:60;overflow:hidden;text-align:left' }, [
    h('div', { style: 'display:flex;align-items:center;justify-content:space-between;padding:10px 14px;border-bottom:1px solid rgba(255,255,255,.09)' }, [
      h('b', { style: 'font-size:.85rem' }, [icon('bell',15),' Notifications']),
  ...
  async function pairModal9() {
    ...
    close9 = openModal(active9 ? '✅ Your chain is ACTIVE — earning is ON' : '⏳ Pair pending — one step from earning', [
      ...
      h('div', { style: 'display:flex;gap:8px;margin-top:12px;flex-wrap:wrap' }, [
        ...
        h('button', { class: 'cp-btn-ghost', onClick: rulesModal9 }, [icon('docs',15),' Full rules']),
      ]),
    ]);
  }
...
function lbStopsModal9(l9) {
  ...
  openModal('🟣 Route — ' + (st9.length + 2) + ' stops total', [
    ...
    h('div', { style: 'margin-top:10px;background:rgba(139,92,246,.1);border:1px solid rgba(139,92,246,.3);border-radius:11px;padding:9px 12px' },
      h('div', { class: 'cp-row-s', style: 'line-height:1.65' }, [icon('finance',15),' Every extra stop carries stop-off pay on the rate card, and each stop gets its own GPS check-in — detention protection applies at EVERY stop, not just pickup and delivery.'])),
  ]);
}
function notCarrier() {
  ...
  (async () => {
    ...
    (async () => {
      ...
      mount(chainW, h('div', null, [
        ...
        h('div', { class: 'cp-row-t', style: 'margin:14px 0 4px' }, [icon('loads',15),' Chain loads — live']),
        ...
        noticeRows.length ? h('div', null, [h('div', { class: 'cp-row-t', style: 'margin:14px 0 4px' }, [icon('bell',15),' Latest activity']), ...noticeRows]) : null,
      ].filter(Boolean)));
    })();
...
async function appView(user) {
  ...
  let tab = (location.hash || '').replace('#', '') || 'dashboard';
  if (tab.indexOf('/') > 0) { const _dl9 = tab.split('/'); tab = _dl9[0]; if (_dl9[1]) window.__lbDeepEnt = { tab: _dl9[0], id: _dl9.slice(1).join('/') }; }  // entity deep links: #loads/<id>, #trips/<id>
  ...
  async function refreshUnread() { try { const ns = await pocketNotifications(50); const u = (ns || []).filter(n => !n.read_at).length; if (u > 0) { bellBadge.textContent = String(u > 9 ? '9+' : u); bellBadge.hidden = false; } else bellBadge.hidden = true;
    setAppBadge(u);
  ...
  function render() {
    ...
    else if (tab === 'rates') import('../shared/market-widget.js').then(function (m) { const hst = h('div'); const fuelHost9 = h('div'); mount(content, h('div', null, [hst, fuelHost9]));
      m.renderMarketWidget(hst, { sub: 'What lanes PAY the truck right now \u2014 real LoadBoot bookings + national benchmarks. Never haul below your cost per mile.' });
      // 2026-08 audit + owner request: fuel finder — DOE/EIA weekly diesel by region (auto-refreshed weekly).
      (async function () { try {
        const fp9 = await fuelPricesGet(); if (!Array.isArray(fp9) || !fp9.length) return;
        const us9 = fp9.find(function (x9) { return x9.region === 'US average'; });
        fuelHost9.appendChild(h('div', { class: 'cp-card', style: 'margin-top:12px' }, [
          cardHead('\u26fd Diesel prices \u2014 DOE weekly survey', us9 ? 'US average $' + Number(us9.diesel).toFixed(3) + '/gal \u00b7 week of ' + us9.as_of : ''),
          h('div', { style: 'display:grid;grid-template-columns:repeat(auto-fill,minmax(150px,1fr));gap:8px' }, fp9.filter(function (x9) { return x9.region !== 'US average'; }).map(function (x9) {
            const hot9 = us9 && Number(x9.diesel) > Number(us9.diesel);
            return h('div', { style: 'border:1px solid rgba(148,163,184,.25);border-radius:12px;padding:10px 12px' }, [
              h('div', { class: 'cp-row-s', style: 'font-weight:700' }, x9.region),
              h('div', { style: 'font-weight:800;font-size:1.05rem;color:' + (hot9 ? '#fbbf24' : '#4ade80') }, '$' + Number(x9.diesel).toFixed(3) + '/gal'),
            ]);
          })),
          h('div', { class: 'cp-row-s', style: 'margin-top:8px;color:#94a3b8' }, 'Fuel your profit engine uses this data \u2014 set your MPG in Settings. Green = at/below national average, amber = above.'),
        ]));
  ...
  function openPrompts() {
    ...
    if (pushSupported()) {
      isPushEnabled().then(on => {
        if (on) return;
        try { if (Date.now() - Number(localStorage.getItem('lb_hide_push') || 0) < 14 * 864e5) return; } catch (_) {}
        if (Notification.permission === 'denied') {
          const dcard = h('div', { class: 'cp-prompt' }, [
            icon('bell', 22),
            h('div', { class: 'cp-prompt-txt' }, [h('b', null, 'Notifications are blocked'), h('span', null, 'Turn them on in your phone Settings \u2192 Apps \u2192 LoadBoot \u2192 Notifications to get load offers and payment alerts.')]),
            h('button', { class: 'cp-prompt-x', onClick: () => { try { localStorage.setItem('lb_hide_push', String(Date.now())); } catch (_) {} dcard.remove(); } }, '\u00d7'),
          ]);
          promptHost.appendChild(dcard);
          return;
        }
        const card = h('div', { class: 'cp-prompt' }, [
          ...
          h('button', { class: 'cp-prompt-x', onClick: () => { try { localStorage.setItem('lb_hide_push', String(Date.now())); } catch (_) {} card.remove(); } }, '×'),
        ]);
        promptHost.appendChild(card);
      });
    }
    if (navigator.geolocation && !(function () { try { return Date.now() - Number(localStorage.getItem('lb_hide_geo') || 0) < 14 * 864e5; } catch (_) { return false; } })()) {
      const card = h('div', { class: 'cp-prompt' }, [
        icon('pin', 22),
        ...
        h('button', { class: 'cp-prompt-x', onClick: () => { try { localStorage.setItem('lb_hide_geo', String(Date.now())); } catch (_) {} card.remove(); } }, '×'),
      ]);
      promptHost.appendChild(card);
    }
  }
  ...
  async function loadHealth() {
    ...
    let ah; try { ah = await accountHealth(); } catch (e) { mount(content, h('div', { class: 'cp-card' }, [h('div', { class: 'cp-muted' }, (e && e.message) || 'Could not load account health.'), h('button', { class: 'cp-btn cp-btn-sm', style: 'margin-top:8px', onClick: () => loadHealth() }, 'Retry')])); return; }
    // 2026-08 audit: visible status tier from REAL delivered/on-time stats (Uber Pro pattern, honest copy).
    const tierCard9 = h('div');
    (async () => {
      try {
        const [tr9, tp9] = await Promise.all([pocketTrips(200).catch(() => []), myTrustProfile().catch(() => null)]);
        const done9 = (tr9 || []).filter(t9 => t9.status === 'delivered' || t9.status === 'invoiced').length;
        const ot9 = tp9 && tp9.on_time_pct != null ? Number(tp9.on_time_pct) : null;
        const TIERS9 = [['Bronze', 0, '#b45309'], ['Silver', 5, '#94a3b8'], ['Gold', 20, '#f59e0b'], ['Platinum', 50, '#7cc0ff']];
        let ti9 = 0; TIERS9.forEach((t9, i9) => { if (done9 >= t9[1] && (i9 < 2 || (ot9 == null || ot9 >= (i9 === 2 ? 90 : 95)))) ti9 = i9; });
        const cur9 = TIERS9[ti9], nxt9 = TIERS9[ti9 + 1] || null;
        const pct9 = nxt9 ? Math.min(100, Math.round(done9 / nxt9[1] * 100)) : 100;
        tierCard9.appendChild(h('div', { class: 'cp-card', style: 'border-color:' + cur9[2] + '55' }, [
          h('div', { style: 'display:flex;align-items:center;gap:10px;flex-wrap:wrap' }, [
            h('span', { style: 'font-size:26px;line-height:1' }, ti9 >= 3 ? '💎' : ti9 === 2 ? '🥇' : ti9 === 1 ? '🥈' : '🥉'),
            h('div', { style: 'flex:1;min-width:160px' }, [
              h('div', { class: 'cp-row-t' }, 'LoadBoot Status: ' + cur9[0]),
              h('div', { class: 'cp-row-s' }, done9 + ' delivered' + (ot9 != null ? ' · ' + ot9 + '% on-time' : '') + (nxt9 ? ' — ' + Math.max(0, nxt9[1] - done9) + ' more to ' + nxt9[0] : ' — top tier')),
            ]),
          ]),
          nxt9 ? h('div', { style: 'height:6px;border-radius:99px;background:rgba(148,163,184,.2);margin-top:10px;overflow:hidden' }, h('i', { style: 'display:block;height:6px;border-radius:99px;width:' + pct9 + '%;background:' + cur9[2] })) : null,
          h('div', { class: 'cp-row-s', style: 'margin-top:8px;color:#94a3b8' }, 'Higher tiers rank you higher in broker matching and direct-offer targeting — built purely from your real delivered record.'),
        ].filter(Boolean)));
      } catch (_) {}
    })();
    ...
    mount(content, h('div', null, [tierCard9, tiles, heroCard, poaCard, strikesCard, actions, groupsCard, ratingCard, reviewsCard, trustCard, networkCard, explain].filter(Boolean)));
  }
  ...
  function loadSettings() {
    ...
    if (pushSupported()) {
      ...
      pushCard = h('div', { class: 'cp-card' }, [
        ...
        h('div', { class: 'cpx-set-row' }, [
          h('div', null, [h('div', { class: 'cpx-set-t' }, [icon('bell',15),' Push notifications']), h('div', { class: 'cpx-set-s' }, 'Trips, payments and announcements on this device')]),
    ...
    const availCard = h('div', { class: 'cp-card' }, [
      ...
      h('div', { class: 'cpx-set-row' }, [
        h('div', null, [h('div', { class: 'cpx-set-t' }, [icon('truck',15),' Available for loads']), h('div', { class: 'cpx-set-s' }, 'Offline = dispatch pauses new load matches for you')]),
        availBtn,
      ]),
    ]);
    ...
    const cpmCard = h('div', { class: 'cp-card' }, [
      ...
      h('div', { class: 'cpx-set-row' }, [
        h('div', null, [h('div', { class: 'cpx-set-t' }, [icon('finance',15),' My all-in cost per mile']), h('div', { class: 'cpx-set-s' }, 'Fuel + truck + insurance + fees per mile. Every load card then shows your estimated profit.')]),
    ...
    // 2026-08 audit (owner-approved): real security controls — TOTP 2FA, global sign-out, data export.
    const secCard = h('div', { class: 'cp-card' }, [h('h3', { class: 'cp-row-t', style: 'margin:0 0 4px' }, '🔐 Security')]);
    (function () {
      const st9 = h('div', { class: 'cp-err' });
      const host9 = h('div');
      async function draw9() {
        host9.innerHTML = '';
        let fs9 = { totp: [] }; try { fs9 = await mfaListFactors(); } catch (_) {}
        const f9 = (fs9.totp || []).find((x9) => x9.status === 'verified');
        if (f9) {
          host9.appendChild(h('div', { class: 'cpx-set-row' }, [h('div', { class: 'cpx-set-t' }, ['Two-factor authentication ', h('span', { class: 'cp-pill green' }, 'ON')]),
            h('button', { class: 'cp-btn cp-btn-sm ghost', style: 'margin-left:auto', onClick: async (e9) => {
              if (!confirm('Turn OFF two-factor authentication? Your account becomes password-only.')) return;
              const b9 = e9.currentTarget; b9.disabled = true; st9.textContent = '';
              try { await mfaUnenroll(f9.id); await draw9(); }
              catch (er9) { st9.textContent = (er9 && er9.message) || 'Could not turn off 2FA — sign out and back in, then retry.'; b9.disabled = false; }
            } }, 'Turn off')]));
        } else {
          host9.appendChild(h('div', { class: 'cpx-set-row' }, [h('div', { class: 'cpx-set-t' }, 'Two-factor authentication (authenticator app)'),
            h('button', { class: 'cp-btn cp-btn-sm', style: 'margin-left:auto', onClick: async (e9) => {
              const b9 = e9.currentTarget; b9.disabled = true; st9.textContent = '';
              try {
                const en9 = await mfaEnrollTotp();
                const code9 = h('input', { class: 'cp-in', inputmode: 'numeric', maxlength: '6', placeholder: '6-digit code from the app', style: 'text-align:center;font-weight:800;letter-spacing:.3em' });
                ...
                const close9 = openModal('🔐 Set up two-factor', [
                  ...
                  code9, em9,
                  h('button', { class: 'cp-btn', style: 'width:100%;margin-top:8px', onClick: async (ev9) => {
                    const vb9 = ev9.currentTarget; vb9.disabled = true; em9.textContent = '';
                    try { await mfaVerify(en9.id, code9.value); close9(); haptic('success'); lbToast('Two-factor authentication is ON — you’ll be asked for a code at sign-in.', 'success'); await draw9(); }
                    catch (er9) { em9.textContent = (er9 && er9.message) || 'Wrong code — try again.'; vb9.disabled = false; }
                  } }, 'Activate 2FA'),
                ].filter(Boolean));
              } catch (er9) { st9.textContent = (er9 && er9.message) || 'Could not start 2FA setup.'; }
              b9.disabled = false;
            } }, 'Enable')]));
        }
        host9.appendChild(h('div', { class: 'cpx-set-row' }, [h('div', { class: 'cpx-set-t' }, 'Sign out on every device (revokes all sessions)'),
          h('button', { class: 'cp-btn cp-btn-sm ghost', style: 'margin-left:auto', onClick: async (e9) => {
            if (!confirm('Sign out of LoadBoot on ALL devices, including this one?')) return;
            e9.currentTarget.disabled = true;
            await signOutEverywhere(); location.reload();
          } }, 'Sign out everywhere')]));
        host9.appendChild(h('div', { class: 'cpx-set-row' }, [h('div', { class: 'cpx-set-t' }, 'Download my data (JSON copy of your account)'),
          h('button', { class: 'cp-btn cp-btn-sm ghost', style: 'margin-left:auto', onClick: async (e9) => {
            const b9 = e9.currentTarget; b9.disabled = true; b9.textContent = 'Preparing…';
            try {
              const grab9 = async (fn9) => { try { return await fn9(); } catch (_) { return null; } };
              const data9 = {
                exported_at: new Date().toISOString(),
                profile: await grab9(() => pocketGetProfile()),
                overview: await grab9(() => pocketOverview()),
                dispatch_preferences: await grab9(() => getDispatchPrefs()),
                notification_preferences: await grab9(() => pocketGetPreferences()),
                trips: await grab9(() => pocketTrips(500)),
                invoices: await grab9(() => pocketInvoices(500)),
                documents: await grab9(() => carrierListDocuments()),
                emergency_contacts: await grab9(() => emergencyContacts()),
                rating: await grab9(() => myRating()),
              };
              downloadBlob(new Blob([JSON.stringify(data9, null, 2)], { type: 'application/json' }), 'loadboot-my-data.json');
              haptic('success');
            } catch (er9) { st9.textContent = (er9 && er9.message) || 'Export failed.'; }
            b9.disabled = false; b9.textContent = 'Download';
          } }, 'Download')]));
        // Device list (informational; per-device revoke = the global sign-out above)
        try {
          const devs9 = await myDevices();
          if (Array.isArray(devs9) && devs9.length) {
            const myKey9 = localStorage.getItem('lb_device_key');
            host9.appendChild(h('div', { class: 'cp-row-s', style: 'margin-top:12px;font-weight:800;color:#94a3b8;text-transform:uppercase;font-size:.68rem;letter-spacing:.06em' }, 'Devices signed in (last 10)'));
            devs9.slice(0, 10).forEach((d9) => {
              host9.appendChild(h('div', { class: 'cpx-set-row' }, [
                h('div', { class: 'cpx-set-t' }, [
                  (d9.label || 'Device') + (d9.device_key === myKey9 ? ' — this device' : ''),
                  h('div', { class: 'cp-row-s', style: 'font-size:.7rem;opacity:.7' }, 'last active ' + new Date(d9.last_seen).toLocaleString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })),
                ]),
                d9.device_key === myKey9 ? h('span', { class: 'cp-pill green', style: 'margin-left:auto' }, 'current') : null,
              ].filter(Boolean)));
            });
          }
        } catch (_) {}
      ...
      draw9();
      secCard.appendChild(host9); secCard.appendChild(st9);
    })();
    // 2026-08 audit: referral program was unreachable for carriers — surface it here.
    const refCard = h('div', { class: 'cp-card' }, [h('h3', { class: 'cp-row-t', style: 'margin:0 0 4px' }, '🎁 Invite & earn'), h('div', { class: 'cp-row-s', style: 'margin-bottom:6px' }, 'Refer carriers, brokers or agents — you earn a share of the dispatch fee on every load they run.')]);
    (async () => { try { const r9 = await myReferral(); if (!r9 || !r9.code) { refCard.style.display = 'none'; return; }
      refCard.appendChild(buildReferralStats(r9));
      if (navigator.share) refCard.appendChild(h('button', { class: 'cp-btn cp-btn-sm', style: 'margin-top:8px', onClick: () => { navigator.share({ title: 'Join me on LoadBoot', text: 'Verified loads, real dispatch, 5% only when you book. Sign up with my link:', url: r9.link }).catch(() => {}); } }, '📤 Share my link'));
    } catch (_) { refCard.style.display = 'none'; } })();
    ...
    mount(content, h('div', null, [themeCard, cpmCard, pushCard, availCard, tabsCard, secCard, refCard, acctCard, delCard].filter(Boolean)));
  }
  ...
  async function loadDashboard() {
    ...
    const rateCard9 = (window.__dashRateables && window.__dashRateables.length) ? (() => {
      ...
      const send9 = h('button', { class: 'cp-btn cp-btn-sm', style: 'margin-top:10px', onClick: async () => {
        ...
        try {
          ...
          if (stars9 >= 5) lbMaybeAskStoreReview();
    ...
    (async () => {
      try {
        ...
        mount(noaDash9, h('div', { class: 'cp-card', style: 'border:1.5px solid rgba(139,92,246,.5);background:rgba(139,92,246,.08)' }, [
          h('div', { style: 'font-weight:900' }, [icon('bank',15),' One step left on factoring — upload your NOA letter']),
    ...
    (async () => {
      ...
      renderAvailabilityCard(availHostD, { h, status: st9, variant: 'dashboard',
        onPost: (k9) => openPostingForm(null, k9 || 'empty'),   // bl_avail_0394 — opens here, no trip to the board
  ...
  // bl_avail_0394 — posting availability is the ONE thing a carrier has to be able to do from
  // wherever they are, and until now the form lived INSIDE the Load Board view: every entry
  // point had to navigate there first and wait for the view to mount. The form is defined here,
  ...
  async function openPostingForm(existing, kindPreset) {
    ...
    // destination chooser, EMPTY vs BACKHAUL, 24h expiry. Edits go through the same form.
        let fleet9 = [], drivers9 = [];
        try { [fleet9, drivers9] = await Promise.all([pocketTrucks().catch(() => []), pocketDrivers().catch(() => [])]); } catch (_) {}
        ...
        if (!fleet9.length || !drivers9.length) {
          const gateSt = { has_truck: fleet9.length > 0, has_driver: drivers9.length > 0 };
          const closeGate = openModal('Post your availability', [fleetGateBody({ h, status: gateSt })]);
          // the gate buttons navigate away — close the modal when the hash changes
        ...
        // Truck is required — no "posting without one".
        const pick = h('select', { class: 'cp-in' }, [h('option', { value: '' }, 'Which truck? *')]
          .concat(fleet9.map(t9 => h('option', { value: t9.id }, 'Unit ' + (t9.unit_no || '?') + (t9.equipment ? ' · ' + t9.equipment : '') + (t9.vin ? ' · VIN …' + String(t9.vin).slice(-6) : ' · no VIN')))));
        if (existing && existing.truck_id) pick.value = existing.truck_id;
        ...
        const paintSpec = async () => {
          ...
          if (!existing) {
            const cur = place.get();
            if (!cur.city && t9.domicile_city) place.set({ city: t9.domicile_city, state: t9.domicile_state || '', zip: t9.domicile_zip || '' });
          ...
          specHost.appendChild(h('div', { style: 'border:1px solid rgba(74,222,128,.22);background:rgba(74,222,128,.05);border-radius:11px;padding:9px 12px;margin:2px 0 4px;font-size:.83rem;line-height:1.6' }, [
            h('div', { style: 'font-weight:800;color:#4ade80;margin-bottom:2px' }, 'Your dispatcher and brokers will see'),
          ...
          if (!t9.vin || String(t9.vin).length !== 17) {
            specHost.appendChild(h('div', { style: 'border:1px solid rgba(248,113,113,.35);background:rgba(248,113,113,.07);border-radius:11px;padding:9px 12px;font-size:.83rem;line-height:1.6;color:#fca5a5' }, [
              'This truck has no valid VIN on file — it cannot be posted until it does. ',
              h('button', { class: 'cp-btn cp-btn-sm ghost', style: 'margin-left:6px', onClick: () => { try { _closePT(); } catch (_) {} truckForm(t9); } }, 'Add VIN'),
            ]));
            return;
          }
        ...
        const save = h('button', { class: 'cp-btn', onClick: async (ev) => {
          const pl = place.get(); const bh = kind.value === 'backhaul';
          if (!pick.value) { lbToast('Pick which truck you are posting.', 'urgent', 'Truck required'); pick.focus(); return; }
          if (!pl.state) { lbToast('Pick the state first, then the city.', 'urgent', 'Where is the truck?'); place.focusState(); return; }
  ...
  // Always live, from the first render of the shell — no hook to wait for, nothing to clear.
  ...
  async function loadLoads() {
    ...
    // One form for both "post a truck" and "edit this posting". Until now the only
      ...
      (postings && postings.length) ? h('div', { style: 'margin-top:10px' }, postings.map(p => {
        ...
        return h('div', null, [h('div', { class: 'cp-row', style: 'flex-wrap:wrap' }, [   // bl_ui_0411: on phones the action row wraps under the text instead of pushing 'Not available' out of the card
          ...
          h('div', { style: 'display:flex;gap:6px;align-items:center;flex-wrap:wrap;max-width:100%;justify-content:flex-end' }, [
            ...
            (p.status !== 'paused' && !p.is_fresh) ? h('button', { class: 'cp-btn cp-btn-sm', title: p.status === 'expired' ? 'Repost this truck for today' : 'Confirm this truck is still available today', onClick: async (ev) => {
              ...
              catch (e) { b.disabled = false; b.textContent = p.status === 'expired' ? 'Repost' : '\u2713 Today'; alert((e && e.message) || 'Failed'); }
            } }, p.status === 'expired' ? 'Repost' : '\u2713 Today') : null,
    ...
    function lbTripBuilder9(list9) {
      ...
      for (const A9 of L9) {
        for (const B9 of L9) {
          if (A9.id === B9.id) continue;
          const stA9 = _lbSt(A9.destination), stB9 = _lbSt(B9.origin);
          if (!stA9 || !stB9 || stA9 !== stB9) continue;
          let dh9 = null;
          if (A9.delivery_lat != null && A9.delivery_lng != null && B9.pickup_lat != null && B9.pickup_lng != null) {
            dh9 = Math.round(havMi(Number(A9.delivery_lat), Number(A9.delivery_lng), Number(B9.pickup_lat), Number(B9.pickup_lng)) * 1.2);
            if (dh9 > 200) continue;
          }
          if (A9.delivery_date && B9.pickup_date && String(B9.pickup_date) < String(A9.delivery_date)) continue;
          const rate9 = Number(A9.rate || 0) + Number(B9.rate || 0);
          const mi9 = Number(A9.miles || 0) + Number(B9.miles || 0) + (dh9 == null ? 60 : dh9);
          combos9.push({ a: A9, b: B9, dh: dh9, rate: rate9, mi: mi9, rpm: mi9 > 0 ? rate9 / mi9 : 0 });
        }
      }
      combos9.sort((x9, y9) => y9.rate - x9.rate);
      const top9 = combos9.slice(0, 6);
      openModal('🔗 Trip Builder — 2-leg tours', [
        h('div', { class: 'cp-row-s', style: 'margin-bottom:10px' }, 'Loads on the board chained into round-style tours: leg 2 starts in the state where leg 1 delivers. Ranked by total revenue — book each leg normally.'),
        top9.length ? h('div', null, top9.map(c9 => h('div', { style: 'border:1px solid rgba(148,163,184,.25);border-radius:14px;padding:12px 14px;margin-bottom:10px' }, [
          h('div', { style: 'display:flex;justify-content:space-between;gap:8px;flex-wrap:wrap;align-items:baseline' }, [
            h('b', { style: 'font-size:.95rem' }, (c9.a.origin || '') + ' → ' + (c9.a.destination || '') + '  ⇒  ' + (c9.b.origin || '') + ' → ' + (c9.b.destination || '')),
            h('b', { style: 'color:#4ade80;font-size:1.05rem' }, money(c9.rate)),
          ]),
          h('div', { class: 'cp-row-s', style: 'margin-top:4px' }, [
            Math.round(c9.mi).toLocaleString() + ' total mi',
            '$' + c9.rpm.toFixed(2) + '/mi combined',
            c9.dh != null ? c9.dh + ' mi connect deadhead' : 'same-state reload',
            c9.a.pickup_date ? 'PU ' + c9.a.pickup_date : null,
          ].filter(Boolean).join(' · ')),
          h('div', { style: 'display:flex;gap:8px;margin-top:8px;flex-wrap:wrap' }, [
            h('button', { class: 'cp-btn cp-btn-sm ghost', onClick: () => showLoadDetail(c9.a.id) }, 'Leg 1 details'),
            h('button', { class: 'cp-btn cp-btn-sm ghost', onClick: () => showLoadDetail(c9.b.id) }, 'Leg 2 details'),
          ]),
        ]))) : h('div', { class: 'cp-muted', style: 'padding:14px 0' }, 'No chainable pairs on the current board — more loads unlock tours automatically. Try widening your filters.'),
      ].filter(Boolean));
    }
        ...
        const render = () => {
          ...
          mount(host, [
            ...
            h('div', { class: 'cpx-adv-grid' }, [
              h('div', { class: 'cell' }, [h('label', null, [icon('pin',15),' DEADHEAD (MI)']), numIn(st.dh, 1, (v) => { st.dh = v; st.dhKnown = true; st.dhBasis = 'entered by you'; render(); }), h('span', { style: 'font-size:9.5px;color:#94a3b8;font-weight:600' }, st.dhBasis)]),
              h('div', { class: 'cell' }, [h('label', null, '⛽ FUEL $/GAL (auto-saved)'), numIn(st.fuel, 0.05, (v) => { st.fuel = v; persistCpm(); render(); })]),
              h('div', { class: 'cell' }, [h('label', null, [icon('truck',15),' TRUCK MPG (auto-saved)']), numIn(st.mpg, 0.1, (v) => { st.mpg = v; persistCpm(); render(); })]),
              h('div', { class: 'cell' }, [h('label', null, [icon('tools',15),' OTHER COST/MI (maint+ins)']), numIn(st.cpm, 0.05, (v) => { st.cpm = v; persistCpm(); render(); })]),
  ...
  async function showLoadDetail(loadId) {
    ...
    // 2026-08 audit: facility ratings (carriers rate pickup/delivery facilities after delivery).
    (async function () { try {
      const o9 = d && (d.origin || (d.load && d.load.origin)), de9 = d && (d.destination || (d.load && d.load.destination));
      if (!o9 && !de9) return;
      const fr9 = await facilityRatings([o9, de9].filter(Boolean));
      const pk9 = fr9 && fr9[String(o9 || '').toLowerCase().trim()], dl9 = fr9 && fr9[String(de9 || '').toLowerCase().trim()];
      if (!pk9 && !dl9) return;
      bodyEl.appendChild(h('div', { class: 'cp-row-s', style: 'margin-top:10px;padding:9px 12px;border:1px solid rgba(245,158,11,.35);border-radius:10px;background:rgba(245,158,11,.07)' },
        '\ud83c\udfed Facility ratings from carriers: ' + [pk9 ? 'pickup ' + pk9.avg + '\u2605 (' + pk9.n + ')' : null, dl9 ? 'delivery ' + dl9.avg + '\u2605 (' + dl9.n + ')' : null].filter(Boolean).join(' \u00b7 ')));
    } catch (_) {} })();
    ...
    mount(bodyEl, h('div', null, [
      ...
      h('div', { class: 'cp-payinfo', style: 'margin-top:10px' }, [h('div', { class: 'cp-payinfo-h' }, [icon('pin',15),' Tracking']), h('div', { class: 'cp-payinfo-b' }, t.tracking_note || 'Location tracking is on from booking until delivery.')]),
    ].filter(Boolean)));
  }
...
function tripStepper(status) {
  ...
  async function loadTrips() {
    showSkeleton(content, 'cards');
    let rows; try { rows = await pocketTrips(80); } catch (e) { mount(content, h('div', { class: 'cp-card' }, [h('div', { class: 'cp-muted' }, 'Could not load your trips — check your connection.'), h('button', { class: 'cp-btn cp-btn-sm', style: 'margin-top:8px', onClick: () => loadTrips() }, 'Retry')])); return; }
    try {
    ...
    mount(content, h('div', { class: 'cp-card', 'data-tour': 'trips-list' }, [cardHead('My trips', rows.length + ' total'), ...rows.map(t => { const cardOf = () => {
      ...
      } }, [icon('alert',15),' Report issue']) : null;
      ...
      const rateBtn = canPod ? h('button', { class: 'cp-btn cp-btn-sm ghost', onClick: () => {
        ...
        const send = h('button', { class: 'cp-btn cp-btn-sm', onClick: async (ev) => {
          ...
          ev.currentTarget.disabled = true; ev.currentTarget.textContent = 'Sending…';
          try { await rateCounterparty(t.id, sel, cmt.value.trim() || null); rateW.innerHTML = ''; rateW.appendChild(h('div', { class: 'cp-row-s', style: 'color:var(--lb-green, #16a34a)' }, '✓ Rating submitted — thank you')); if (sel >= 5) lbMaybeAskStoreReview();
            // 2026-08 audit: also rate the facilities (one tap per stop) — builds the crowd-sourced facility data.
            [['pickup', '\ud83d\udce6 Rate pickup facility'], ['delivery', '\ud83d\udce4 Rate delivery facility']].forEach(function (fk9) {
              const frow9 = h('div', { class: 'cp-row-s', style: 'margin-top:6px;display:flex;gap:6px;align-items:center;flex-wrap:wrap' }, [h('span', null, fk9[1] + ':')]);
              for (let st9 = 1; st9 <= 5; st9++) {
                frow9.appendChild(h('button', { class: 'cp-btn cp-btn-sm ghost', style: 'padding:2px 8px', onClick: async function (ev9) {
                  const b9 = ev9.currentTarget; try { await facilityReviewSubmit(t.id, fk9[0], st9, null); frow9.innerHTML = ''; frow9.appendChild(h('span', { style: 'color:var(--lb-green, #16a34a)' }, '\u2713 ' + fk9[1].replace('Rate ', '') + ' rated ' + st9 + '\u2605')); haptic('tap'); } catch (e9) { b9.textContent = '!'; }
                } }, st9 + '\u2605'));
              }
              rateW.appendChild(frow9);
            }); }
      ...
      const assign = active ? h('button', { class: 'cp-btn cp-btn-sm ghost', onClick: () => assignTrip(t) }, [icon('user',15),' Assign driver/truck']) : null;
      ...
      } }, [icon('clock',15),' Arrive / depart']) : null;
      ...
      } }, [icon('clipboard',15),' Dispatch sheet']);
      ...
      } }, [icon('loads',15),' Delivery docs']) : null;
      ...
      // 2026-08 audit: Uber-pattern contact action right on the trip card.
      const contactBtn = h('button', { class: 'cp-btn cp-btn-sm ghost', onClick: () => {
        let close9 = null;
      ...
      const tonuEvidence = (_cancelled && t.cancel_fault && t.cancelled_by === 'broker') ? (_faultCarrier
        ? h('div', { style: 'margin-top:9px;padding:11px 13px;border-radius:11px;background:rgba(239,68,68,.12);border:1px solid rgba(239,68,68,.4)' }, [
            ...
            h('div', { style: 'margin-top:8px;padding:9px 11px;border-radius:9px;background:rgba(0,0,0,.25)' }, [
              h('div', { style: 'font-weight:700;font-size:.78rem;color:#e5edf8;margin-bottom:3px' }, [icon('pin',15),' GPS evidence recorded at cancellation']),
      ...
      return h('div', { class: 'cp-trip', 'data-trip': String(t.id || '') }, [
        hero,
  ...
  async function showCarrierPod(t, host) {
    ...
    function podUploader(state) {
      ...
      function pick(f) {
        ...
        else mount(prev, h('div', null, [h('div', { class: 'cp-podpdf' }, [icon('docs',15),' PDF ready to upload']), meta, rm]));
        up.disabled = false;
      }
      ...
      // 2026-08 audit: scan the paper POD with the camera — photos become one PDF, straight into the picker.
      const camIn = h('input', { type: 'file', accept: 'image/*', capture: 'environment', multiple: true, style: 'display:none' });
      camIn.onchange = async () => {
        const fs9 = Array.from(camIn.files || []); if (!fs9.length) return;
        err.textContent = ''; prog.textContent = 'Building PDF from ' + fs9.length + ' photo' + (fs9.length > 1 ? 's' : '') + '…';
        try { const blob9 = await imagesToPdf(fs9); prog.textContent = ''; pick(new File([blob9], 'POD-scan-' + Date.now() + '.pdf', { type: 'application/pdf' })); haptic('success'); }
        catch (_) { prog.textContent = ''; err.textContent = 'Could not build the PDF — try again or pick a file instead.'; }
        camIn.value = '';
      };
      const camBtn9 = h('button', { class: 'cp-btn cp-btn-sm ghost', style: 'width:100%;margin-top:8px', onClick: () => camIn.click() }, '📷 Scan paper POD with camera');
      ...
      mount(wrap, [
        ...
        zone, camBtn9, camIn, input, prev, err, prog,
  ...
  async function loadFleet() {
    ...
    function _truckFormInner(t) {
      ...
      // --- the trailer (open-deck combos) ---
      // A hotshot's capacity IS the trailer: the F-250 tows it, the Big Tex carries
      // the freight. Until now the form knew everything about the truck and nothing
      // about the trailer — which is why posting Munster's unit on a load board
      // meant asking the carrier on WhatsApp for deck length and payload. These show
      // only for open-deck equipment; a dry van keeps its uncluttered form.
      const trType = h('select', { class: 'cp-in' }, [['', 'Trailer type…'], ['gooseneck', 'Gooseneck'], ['bumper_pull', 'Bumper pull'], ['deckover', 'Deckover'], ['lowboy', 'Lowboy'], ['none', 'No trailer — deck on chassis']].map(o => h('option', { value: o[0], selected: t && t.trailer_type === o[0] ? 'selected' : null }, o[1])));
      const trLen = numIn('Deck length (ft) — from the trailer plate', t && t.trailer_len_ft);
      const trVin = h('input', { class: 'cp-in', placeholder: 'Trailer VIN (17 characters) — it is on your COI schedule', value: (t && t.trailer_vin) || '' });
      const tarps = check('Tarps on board', t && t.has_tarps);
      const chains = check('Chains & binders', t && t.has_chains);
      const OPEN_DECK = ['Hotshot', 'Flatbed', 'Step Deck', 'Power Only', 'Conestoga'];
      const trailerSec = h('div', { style: 'display:none' }, [
        sec('The trailer', 'For open-deck work the trailer IS the capacity — every load board asks for this before your truck can be posted.'),
        trType, row2(trLen, trVin),
        h('div', { style: 'display:grid;grid-template-columns:1fr 1fr;gap:0 12px' }, [tarps, chains]),
      ]);
      const syncTrailer = () => { trailerSec.style.display = OPEN_DECK.indexOf(eq.value) >= 0 ? '' : 'none'; };
      eq.addEventListener('change', syncTrailer); syncTrailer();

      // Posting readiness — the fields a dispatcher must have before this unit can go
      ...
      const readyCheck = () => {
        ...
        if (!numOf(cLen) && !numOf(trLen)) missing.push('deck/cargo length');
        if (!(dZip.value.trim() || (dCity.value.trim() && dState.value.trim()))) missing.push('home base');
        if (OPEN_DECK.indexOf(eq.value) >= 0 && !trType.value) missing.push('trailer type');
        if (missing.length) { postReady.textContent = '\u26a0 Not posting-ready \u2014 load boards will ask for: ' + missing.join(', '); postReady.style.color = '#fbbf24'; }
        else { postReady.textContent = '\u2713 Posting-ready \u2014 this unit can go on a load board as-is.'; postReady.style.color = '#4ade80'; }
      };
      [payload, cLen, trLen, dZip, dCity, dState].forEach((el2) => el2.addEventListener('input', readyCheck));
      [eq, trType].forEach((el2) => el2.addEventListener('change', readyCheck));
      ...
      const save = h('button', { class: 'cp-btn cp-btn-sm', onClick: async (ev) => {
        ...
        try {
          await pocketUpsertTruck({
            ...
            trailerType: trType.value || null, trailerLenFt: numOf(trLen), trailerVin: trVin.value.trim().toUpperCase() || null,
            hasTarps: tarps._box.checked, hasChains: chains._box.checked,
          });
    ...
    const optCard9 = (() => {
      const host9 = h('div', { class: 'cp-card' }, [cardHead([icon('chart',15),' Fleet optimization'], 'last 90 days'), h('div', { class: 'cp-muted' }, 'Loading…')]);
      (async () => {
        ...
        if (!tr9.length) { mount(host9, [cardHead([icon('chart',15),' Fleet optimization'], 'last 90 days'), h('div', { class: 'cp-muted' }, 'Analytics appear after your first delivered trips — per-truck utilization, $/mile and your best lanes.')]); return; }
  ...
  function taxCenter() {
    ...
    (async () => {
      ...
      mount(card, h('div', null, [
        ...
        h('div', { class: 'cp-row-s', style: 'font-weight:800;margin:12px 0 2px' }, [icon('chart',15),' Schedule C — deductible expenses logged']),
  ...
  function cashCard(rows) {
    ...
    return h('div', { class: 'cp-card' }, [
      cardHead([icon('inbox',15),' Cash position'], 'trucking pays in 30–45 days — watch the gap'),
  ...
  function earningsHub() {
    ...
    async function render() {
      ...
      catch (e) { mount(card, [cardHead([icon('finance',15),' Earnings']), h('div', { class: 'cp-muted' }, (e && e.message) || 'Could not load.')]); return; }
      ...
      mount(card, h('div', null, [
        h('div', { style: 'display:flex;justify-content:space-between;align-items:center;gap:8px;flex-wrap:wrap' }, [
          h('div', { class: 'cp-row-t', style: 'font-size:1.05rem' }, [icon('finance',15),' Earnings — every trip, A to Z']),
  ...
  async function loadFinance() {
    showSkeleton(content, 'cards');
    let rows; try { rows = await pocketInvoices(100); } catch (e) { mount(content, h('div', { class: 'cp-card' }, [h('div', { class: 'cp-muted' }, 'Could not load your finance data — check your connection.'), h('button', { class: 'cp-btn cp-btn-sm', style: 'margin-top:8px', onClick: () => loadFinance() }, 'Retry')])); return; }
    rows = rows || [];
    ...
    const factoringCard9 = (() => {
      const host9 = h('div', { class: 'cp-card', style: 'margin-top:14px' }, [h('div', { class: 'cp-cardhead' }, [icon('finance', 18), h('h3', null, [icon('bank',15),' Factoring — how brokers pay you'])]), h('div', { class: 'cp-muted' }, 'Loading…')]);
      (async () => {
        ...
        mount(host9, h('div', null, [
          h('div', { class: 'cp-cardhead' }, [icon('finance', 18), h('h3', null, [icon('bank',15),' Factoring — how brokers pay you'])]),
    ...
    const payFlightCard = h('div', { class: 'cp-card' }, [cardHead([icon('finance',15),' Payments in flight']), h('div', { class: 'cp-muted' }, 'Loading…')]);
    (async () => {
      let trs; try { trs = await payMyTransfers(); } catch (e) { mount(payFlightCard, [cardHead([icon('finance',15),' Payments in flight']), h('div', { class: 'cp-muted' }, (e && e.message) || 'Could not load.')]); return; }
      ...
      const owedRow9 = (x9) => {
        ...
        return h('div', { class: 'cp-row', style: 'flex-wrap:wrap' }, [
          h('div', { style: 'flex:1;min-width:200px' }, [
            h('div', { class: 'cp-row-t' }, [icon('inbox',15),' ' + (x9.label || x9.kind) + ' · ' + money(x9.amount)]),
          ...
          h('div', { class: 'cp-inflight-actions' }, [
            h('span', { class: 'cp-pill', style: 'background:rgba(239,68,68,.14);color:#f87171' }, [icon('clock',13),' awaiting broker payment']),
            routeChip9(x9),
            ...
            } }, [icon('bell',16),' Request payment']) : null,
            ...
            } }, [icon('alert',16),' Dispute non-payment']) : null,
          ].filter(Boolean)),
        ]);
      };
      const row9 = (x) => {
      ...
      mount(payFlightCard, [cardHead([icon('finance',15),' Your money — owed & in flight']),
    ...
    const acctExportCard9 = (() => {
      ...
      return h('div', { class: 'cp-card' }, [cardHead([icon('upload',15),' Accounting export'], 'QuickBooks-compatible CSVs — invoices, expenses, payments'),
    ...
    mount(content, h('div', null, [
      ...
      h('div', { class: 'cp-card' }, [cardHead([icon('receipt',15),' LoadBoot fee invoices'], 'the flat 5% dispatch fee — nothing else'),
        ...
        const packetBtn9 = i.trip_id ? h('button', { class: 'cp-btn-ghost cp-btn-sm', style: 'margin:4px 6px 0 0;font-size:.74rem', onClick: async () => {
          ...
          openModal('📦 Factoring packet — ' + ((pk9.trip && pk9.trip.origin) || '') + ' → ' + ((pk9.trip && pk9.trip.destination) || ''), [
            ...
            (pk9.missing && pk9.missing.length) ? h('div', { style: 'background:rgba(245,158,11,.1);border:1px solid rgba(245,158,11,.35);border-radius:11px;padding:10px 13px;margin-bottom:10px' },
              [h('b', { style: 'color:#fbbf24' }, [icon('alert',15),' Still needed for funding']), ...pk9.missing.map((m9) => li9('• ' + m9))]) : h('div', { class: 'cp-pill', style: 'background:rgba(34,197,94,.15);color:#4ade80;font-weight:800;margin-bottom:10px' }, [icon('check',15),' Packet complete — fund-ready']),
        ...
        } }, [icon('loads',15),' Factoring packet']) : null;
        ...
        const payFee = i.status === 'sent' ? h('button', { class: 'cp-btn cp-btn-sm', style: 'background:#16a34a', onClick: async () => {
          ...
          dw.appendChild(h('div', { style: 'margin-top:8px;border:1px solid rgba(255,255,255,.14);border-radius:16px;overflow:hidden' }, [
            h('div', { style: 'background:linear-gradient(120deg,#10223B,#0d2f56);padding:12px 16px;display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap' }, [
              h('div', null, [h('div', { style: 'font-weight:900;color:#fff' }, [icon('card',15),' Pay LoadBoot']), h('div', { style: 'font-size:.72rem;color:#9fb0cc' }, 'flat 5% dispatch fee · net 15')]),
            ...
            h('div', { style: 'padding:12px 16px' }, [
              h('div', { style: 'background:rgba(8,131,247,.08);border:1px solid rgba(8,131,247,.3);border-radius:12px;padding:10px 13px' }, [
                h('div', { style: 'font-weight:800;font-size:.78rem;letter-spacing:.06em;color:#93c5fd;margin-bottom:4px' }, [icon('bank',15),' REMIT-TO']),
              ...
              rules9 ? h('details', { style: 'margin-bottom:8px' }, [h('summary', { class: 'cp-row-s', style: 'cursor:pointer;font-weight:700' }, [icon('docs',15),' Payment rules']), h('div', { class: 'cp-row-s', style: 'white-space:pre-wrap;line-height:1.7;margin-top:4px' }, rules9)]) : null,
  ...
  async function loadDocuments() {
    ...
    (async () => {
      try {
        ...
        const c9 = await caches.open('lb-share-inbox');
        const keys9 = await c9.keys();
    ...
    try {
      ...
      if (pp9 && pp9.factoring_noa && !latestDoc('noa')) {
        noaBanner9 = h('div', { class: 'cp-card', style: 'border:1.5px solid rgba(139,92,246,.5);background:rgba(139,92,246,.08);margin-bottom:12px' }, [
          h('div', { style: 'font-weight:900' }, [icon('bank',15),' Upload your factor\u2019s NOA letter']),
  ...
  async function loadSupport() {
    ...
    const msg = h('div', { class: 'cp-err' });
    const list = h('div');
    const send = h('button', { class: 'cp-btn', onClick: async () => { msg.textContent = ''; if (!subj.value.trim()) { msg.textContent = 'Subject is required.'; return; } send.disabled = true; send.textContent = 'Sending…'; try { const _a9 = (send._getAtts ? send._getAtts() : []); const _bodyF = body.value.trim() + (_a9.length ? '\n\nATTACHED DOCUMENTS (' + _a9.length + '): ' + _a9.map((x) => x.file_name).join(' \u00b7 ') + ' \u2014 files are in document review.' : ''); await pocketRaiseIssue(subj.value.trim(), _bodyF); subj.value = ''; body.value = ''; msg.className = 'cp-err ok'; msg.textContent = 'Sent — we’ll get back to you.'; await loadIssues(); } catch (e) {
      const m9 = String((e && e.message) || '');
      if (!navigator.onLine || /fetch|network|timeout/i.test(m9)) {
        // 2026-08 audit: offline outbox — the ticket is queued and auto-sent on reconnect.
        try { const q9 = JSON.parse(localStorage.getItem('lb_ticket_outbox') || '[]'); q9.push({ s: subj.value.trim(), b: _bodyF, t: Date.now() }); localStorage.setItem('lb_ticket_outbox', JSON.stringify(q9.slice(-10))); subj.value = ''; body.value = ''; msg.className = 'cp-err ok'; msg.textContent = '📮 You appear offline — the ticket is saved and will send automatically when you reconnect.'; } catch (_) { msg.className = 'cp-err'; msg.textContent = m9 || 'Could not send.'; }
      } else { msg.className = 'cp-err'; msg.textContent = m9 || 'Could not send.'; }
    } send.disabled = false; send.textContent = 'Send to dispatch'; } }, 'Send to dispatch');
    const exList = h('div');
    ...
    // 2026-08 audit: in-app searchable Help Center (Uber/DoorDash pattern) — FAQ tier before the ticket tier.
    const faqCard9 = h('div', { class: 'cp-card' }, [cardHead('Help center', 'Instant answers — search before you wait on a reply')]);
    try { renderFaq(faqCard9, CARRIER_FAQ); } catch (_) {}
    mount(content, h('div', null, [deskCard, faqCard9, h('div', { class: 'cp-grid' }, [
  ...
  // ----- FMCSA screen -------------------------------------------------------
  // This used to be one boolean:
  //     __disq = authority === 'inactive' || allowedToOperate === 'N' || outOfService
  // which got three separate things wrong.
  //
  // 1. It never looked at brokerOnly, even though the edge function reads FMCSA
  //    Licensing & Insurance and returns it. A freight broker holds broker
  //    authority and no carrier authority, so the function reports authority as
  //    "inactive" — and a broker did get stopped, but by accident, and was then
  //    told "your authority is inactive / out of service". That is false. Their
  //    authority is perfectly active; it is broker authority. We dispatch
  //    carriers, and a broker signing up as a carrier is how double-brokering
  //    starts, so this needs its own verdict and its own sentence.
  // 2. It failed open on "unknown". FMCSA's L&I endpoint returns nothing for a
  //    freshly granted docket, and for at least one path it returns nothing for
  //    a large broker too — Total Quality Logistics entered by DOT is stopped,
  //    the same company entered by MC walks straight through. Unknown is not a
  //    pass. We let them continue (an unreadable federal API is not the
  //    carrier's fault) but the account is flagged so review sees it.
  // 3. It said nothing about an entity holding BOTH authorities. That is legal
  //    and common, and not a block — but review should know.
  //
  // Returns a verdict, never a bare boolean, so every caller says the true thing.
  function lbFmcsaScreen(res) {
    const cr = (res && (res.carrier || res.result)) || null;
    if (!cr) {
      return { block: false, code: 'unreadable', flag: true,
        title: 'FMCSA did not answer',
        msg: 'We could not reach FMCSA to check this authority. You can continue — but your account goes to our compliance team for a manual check before you can book.' };
    }
    if (cr.brokerOnly === true) {
      return { block: true, code: 'broker_only', flag: true,
        title: 'This is a broker authority, not a carrier authority',
        msg: 'FMCSA Licensing & Insurance shows ' + (cr.legalName || 'this entity') + ' holding ACTIVE broker authority and no motor carrier authority'
             + (cr.mcNumber ? ' (docket ' + cr.mcNumber + ')' : '') + '. Nothing is wrong with your registration — but LoadBoot dispatches carriers, and a broker cannot be onboarded as one. If you also hold motor carrier authority under a different USDOT, enter that number instead. If you want to post freight to our carriers, that is a broker account, not this one.' };
    }
    if (cr.outOfService === true) {
      return { block: true, code: 'out_of_service', flag: true,
        title: 'Out of service',
        msg: 'FMCSA shows this USDOT under an out-of-service order' + (cr.oosDate ? ' dated ' + cr.oosDate : '') + '. We cannot onboard while that stands — resolve it with FMCSA first.' };
    }
    if (cr.allowedToOperate === 'N') {
      return { block: true, code: 'not_allowed', flag: true,
        title: 'Not allowed to operate',
        msg: 'FMCSA lists this USDOT as not allowed to operate. Onboarding is blocked until that clears.' };
    }
    if (cr.authority === 'inactive' && cr.authorityVerified === true) {
      return { block: true, code: 'authority_inactive', flag: true,
        title: 'Operating authority is not active',
        msg: 'FMCSA Licensing & Insurance shows no active motor carrier authority on this USDOT. Onboarding is blocked until FMCSA shows it active again.' };
    }
    if (cr.brokerAuthority === true && cr.carrierAuthority === true) {
      return { block: false, code: 'dual_authority', flag: true,
        title: 'Carrier and broker authority',
        msg: 'You hold both motor carrier and broker authority. That is fine — you are onboarding as a carrier, and loads we dispatch to you must be run on your own authority, not re-brokered.' };
    }
    if (cr.authorityVerified !== true || cr.authority === 'unknown') {
      return { block: false, code: 'authority_unverified', flag: true,
        title: 'Authority could not be confirmed',
        msg: 'FMCSA has not published authority records for this number yet — normal for a docket granted in the last few weeks. You can continue; our compliance team confirms it from your authority letter before you book.' };
    }
    if (cr.brokerHint === 'no_power_units') {
      return { block: false, code: 'no_power_units', flag: true,
        title: 'No trucks on file with FMCSA',
        msg: 'Your MCS-150 reports no power units. Continue, but compliance will ask about it — we can only dispatch equipment that exists on the federal record.' };
    }
    return { block: false, code: 'ok', flag: false };
  }
    ...
    function draw() {
      ...
      if (st === 0) {
        ...
        const resCard = fmcsaRes ? h('div', { class: 'cp-card', style: 'margin-top:8px' }, [
          ...
          h('div', { class: 'cp-row' }, [h('span', null, 'Safety rating'), h('span', null, String(g('safety_rating') || 'none'))]),
          (fmcsaRes && fmcsaRes.__screen && (fmcsaRes.__screen.block || fmcsaRes.__screen.flag))
            ? h('div', { class: 'cp-ann ' + (fmcsaRes.__screen.block ? 'emergency' : 'warning'), style: 'margin-top:8px' },
                [h('div', { class: 'cp-ann-t' }, fmcsaRes.__screen.title), h('div', { class: 'cp-ann-b' }, fmcsaRes.__screen.msg)])
            : null,
        ...
        const vbtn = h('button', { class: 'cp-btn cp-btn-sm', style: 'margin-top:6px', onClick: async (ev) => { const _vb = ev.currentTarget;
          ...
          try { let dd; try { dd = await fmcsaVerify({ mc: mc || null, dot: dot || null }); } catch (e1) { if (/not responding|taking too long|Could not reach/i.test((e1 && e1.message) || '')) { _vb.textContent = 'FMCSA slow \u2014 retrying\u2026'; await new Promise((r) => setTimeout(r, 1500)); dd = await fmcsaVerify({ mc: mc || null, dot: dot || null }); } else { throw e1; } } fmcsaRes = dd || {}; const cr = fmcsaRes.carrier || fmcsaRes.result || fmcsaRes; const nm = cr && (cr.legalName || cr.legal_name || cr.dbaName || cr.name); if (nm) f.company = nm; if (cr && cr.phone && !String(f.phone || '').trim()) f.phone = String(cr.phone); if (cr && cr.mcNumber) f.mc = String(cr.mcNumber).replace(/\D/g, ''); if (cr && cr.dotNumber) f.dot = String(cr.dotNumber);
            fmcsaRes.__screen = lbFmcsaScreen(fmcsaRes);
            fmcsaRes.__disq = !!fmcsaRes.__screen.block;
            if (fmcsaRes.__screen.block || fmcsaRes.__screen.flag) lbToast(fmcsaRes.__screen.msg, fmcsaRes.__screen.block ? 'urgent' : 'action', fmcsaRes.__screen.title);
            draw(); }
      ...
      else if (st === 1) { const eq = h('div', { class: 'cp-eqgrid' }, EQUIP.map(e => { const on = (f.equipment_types || []).includes(e); const b = h('button', { class: 'cp-chip2' + (on ? ' on' : ''), onClick: () => { const s = new Set(f.equipment_types || []); if (s.has(e)) s.delete(e); else s.add(e); f.equipment_types = [...s]; b.classList.toggle('on'); } }, e); return b; })); body = h('div', { class: 'cp-wiz-grid' }, [field('Home base (city, ST)', 'home_base', 'Dallas, TX'), field('Search radius (miles)', 'radius_miles', '300', 'number'), field('Number of trucks', 'truck_count', '1'), h('div', { class: 'cp-fld' }, [h('span', { class: 'cp-row-t' }, 'Equipment types'), eq]), h('div', { class: 'cp-fld' }, [
        h('span', { class: 'cp-row-t' }, 'Who drives?'),
        h('div', { class: 'cp-row-s', style: 'margin-bottom:6px' }, 'An owner-operator drives their own truck. With a hired driver you are a fleet owner, and different loads suit you.'),
        h('div', { style: 'display:flex;gap:7px;flex-wrap:wrap' }, [
          ['owner', 'I drive'], ['employed', 'My drivers do'], ['both', 'Both'],
        ].map(([v, lbl]) => { const b = h('button', { class: 'cp-chip2' + (f.owner_drives === v ? ' on' : ''), onClick: () => {
            f.owner_drives = v;
            b.parentNode.querySelectorAll('.cp-chip2').forEach((x) => x.classList.remove('on'));
            b.classList.add('on');
          } }, lbl); return b; })),
      ]),
        ...
        (String(f.factoring_status || '') === 'yes' ? h('div', { style: 'margin-top:12px;border:1.5px solid rgba(139,92,246,.4);background:rgba(139,92,246,.07);border-radius:14px;padding:12px 14px' }, [
          ...
          (() => {
            ...
            return h('div', { style: 'margin-top:10px' }, [
              h('div', { class: 'cp-row-t' }, [icon('docs',15),' The NOA letter itself (PDF from your factoring company)']),
      ...
      const next = st < STEPS.length - 1
        ? h('button', { class: 'cp-btn cp-btn-sm', onClick: async (ev) => { const _btn = ev.currentTarget; _btn.disabled = true; _btn.textContent = 'Saving…'; try { if (st === 0 && (!String(f.company || '').trim() || !String(f.contact_name || '').trim() || (!String(f.mc || '').trim() && !String(f.dot || '').trim()))) { throw new Error('Company name, your name, and MC or DOT number are required to continue.'); } if (st === 0 && !fmcsaRes && (String(f.dot || '').trim() || String(f.mc || '').trim())) {
            ...
            try { const dd0 = await fmcsaVerify({ mc: String(f.mc || '').trim() || null, dot: String(f.dot || '').trim() || null }); fmcsaRes = dd0 || {}; fmcsaRes.__screen = lbFmcsaScreen(fmcsaRes); fmcsaRes.__disq = !!fmcsaRes.__screen.block; } catch (_) { /* FMCSA unreachable — recorded as a review flag below, not treated as a pass */ fmcsaRes = { __screen: lbFmcsaScreen(null), __disq: false }; }
          }
          if (st === 0 && fmcsaRes && fmcsaRes.__disq) { throw new Error((fmcsaRes.__screen && fmcsaRes.__screen.msg) || 'FMCSA shows this authority as INACTIVE or out of service \u2014 onboarding is blocked until your authority is active again.'); }
          if (st === 1) {
            const miss1 = [];
  ...
  async function loadSafety() {
    ...
    const callRow = h('div', { style: 'display:flex;gap:8px;margin-bottom:12px' }, [
      h('a', { class: 'cp-btn', href: 'tel:911', style: 'flex:1;text-align:center;background:#dc2626;text-decoration:none' }, [icon('phone',15),' Call 911']),
    ...
    const contactsCard = h('div', { class: 'cp-card' }, [cardHead('My emergency contacts'),
      ...contacts.map(c => h('div', { class: 'cp-row' }, [
        ...
        h('div', { style: 'display:flex;gap:6px' }, [
          ...
          h('button', { class: 'cp-btn cp-btn-sm ghost', onClick: async () => { if (!confirm('Remove ' + c.name + ' from your emergency contacts?')) return; try { await emergencyContactDelete(c.id); haptic('warn'); loadSafety(); } catch (_) {} } }, '✕'),
        ]),
      ])),
...
/* ---- 2FA login gate (2026-08 audit, owner-approved): aal1 session + enrolled factor -> ask for the code ---- */
function renderMfaGate(factorId) {
  const err9 = h('div', { class: 'cp-err' });
  const code9 = h('input', { class: 'cp-in', inputmode: 'numeric', autocomplete: 'one-time-code', maxlength: '6', placeholder: '6-digit code', style: 'font-size:1.3rem;letter-spacing:.35em;text-align:center;font-weight:800' });
  const go9 = h('button', { class: 'cp-btn cp-btn-lg', style: 'width:100%;margin-top:10px' }, 'Verify');
  const submit9 = async () => {
    const c9 = code9.value.trim(); if (c9.length < 6) { err9.textContent = 'Enter the 6-digit code from your authenticator app.'; return; }
    go9.disabled = true; go9.textContent = 'Verifying…'; err9.textContent = '';
    try { await mfaVerify(factorId, c9); haptic('success'); boot(); }
    catch (e9) { err9.textContent = (e9 && e9.message) || 'Wrong code — try again.'; go9.disabled = false; go9.textContent = 'Verify'; code9.value = ''; code9.focus(); }
  };
  go9.onclick = submit9; code9.onkeydown = (e9) => { if (e9.key === 'Enter') submit9(); };
  mount(root, h('div', { class: 'cp-auth' }, h('div', { class: 'cp-auth-card' }, [
    h('h1', null, '🔐 Two-factor check'),
    h('p', { class: 'cp-auth-sub' }, 'This account is protected with 2FA. Enter the code from your authenticator app.'),
    code9, err9, go9,
    h('button', { class: 'cp-btn ghost', style: 'width:100%;margin-top:10px', onClick: async () => { await signOut(); location.reload(); } }, 'Use a different account'),
  ])));
  root.setAttribute('aria-busy', 'false');
  setTimeout(() => code9.focus(), 60);
}
...
async function boot() {
  ...
  async function flushTicketOutbox9() {
    ...
    if (!q9.length || !navigator.onLine) return;
    const rest9 = [];
    for (const t9 of q9) { try { await pocketRaiseIssue(t9.s, t9.b); } catch (_) { rest9.push(t9); } }
    try { localStorage.setItem('lb_ticket_outbox', JSON.stringify(rest9)); } catch (_) {}
    if (q9.length && !rest9.length) lbToast('Your queued support ticket' + (q9.length > 1 ? 's were' : ' was') + ' sent.', 'success');
  }
  try { flushTicketOutbox9(); window.addEventListener('online', () => setTimeout(flushTicketOutbox9, 1500)); } catch (_) {}
  try { showWhatsNew(['Saved loads (★) + filters that remember themselves on the Load Board', 'Pull down on any page to refresh', 'Scan paper PODs with your camera — straight to upload', 'New Help Center with instant answers in Support', 'Unread badges on Alerts and your app icon']); } catch (_) {}
  try { const f9 = await mfaRequired(); if (f9) { renderMfaGate(f9); return; } } catch (_) {}
