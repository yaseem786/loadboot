...
.dd-grid2{display:grid;grid-template-columns:1fr 1fr;gap:14px}
...
.dd-thread{max-height:300px;overflow:auto;padding:4px 2px;margin:8px 0;display:grid;gap:6px;align-content:start}
...
@media (max-width:760px){.dd-grid2{grid-template-columns:1fr}.dd-actions{grid-template-columns:1fr}.dd-steps{grid-template-columns:repeat(5,minmax(0,1fr))}.dd-step{font-size:.62rem}.dd-hero{padding:18px 16px}.dd-h1{font-size:1.25rem}.dd-count b{font-size:2.1rem}}
`;
...
export async function renderDispatcherDesk(host) {
      ...
      h('div', { class: 'dd-person' }, [
        ...
        h('div', { style: 'flex:1;min-width:200px' }, [
          ...
          h('div', { class: 'dd-badges' }, [
            ...
            trial ? pill('Monitored trial' + (dp.trial_end ? ' · to ' + day(dp.trial_end) : ''), 'blue') : pill('Confirmed dispatcher', 'blue'),
  ...
  if (a && released) {
    ...
    contact = card('Reach ' + (dp.first_name || 'your dispatcher'), 'Company line and mailbox — every call and message is logged with LoadBoot', h('div', null, [
      h('div', { class: 'dd-actions' }, [
        c.phone ? h('a', { class: 'dd-act call', href: 'tel:' + c.phone }, [h('span', { class: 't' }, [icon('phone', 15), ' Call or text']), h('span', { class: 'v' }, c.phone), h('span', { class: 'v', style: 'font-size:.7rem' }, c.phone_label || 'LoadBoot line')]) : h('div', { class: 'dd-act call', style: 'opacity:.55' }, [h('span', { class: 't' }, [icon('phone', 15), ' Call or text']), h('span', { class: 'v' }, 'Line being provisioned — use the thread below')]),
        wa ? h('a', { class: 'dd-act wa', href: 'https://wa.me/' + wa, target: '_blank', rel: 'noopener' }, [h('span', { class: 't' }, [icon('chat', 15), ' WhatsApp']), h('span', { class: 'v' }, c.whatsapp), h('span', { class: 'v', style: 'font-size:.7rem' }, 'LoadBoot dispatch WhatsApp')]) : h('div', { class: 'dd-act wa', style: 'opacity:.55' }, [h('span', { class: 't' }, [icon('chat', 15), ' WhatsApp']), h('span', { class: 'v' }, 'Not available yet')]),
        c.email ? h('a', { class: 'dd-act mail', href: 'mailto:' + c.email }, [h('span', { class: 't' }, [icon('mail', 15), ' E-mail']), h('span', { class: 'v' }, c.email), h('span', { class: 'v', style: 'font-size:.7rem' }, 'Company mailbox')]) : h('div', { class: 'dd-act mail', style: 'opacity:.55' }, [h('span', { class: 't' }, [icon('mail', 15), ' E-mail']), h('span', { class: 'v' }, 'Mailbox being set up')]),
      ]),
      h('div', { class: 'cp-row-s', style: 'margin-top:10px' }, ['Released by LoadBoot compliance ', when(a.contact_released_at), c.escalation_email ? [' · Not happy? Escalate to ', h('a', { href: 'mailto:' + c.escalation_email, style: 'color:#8ec5ff' }, c.escalation_email)] : null]),
    ]));
  } else if (a) {
    contact = card('Direct line', null, h('div', { class: 'dd-lock' }, [
      h('div', { class: 'ic' }, icon('lock', 18)),
      h('div', null, [
        h('b', { style: 'color:#eaf1fb' }, 'Call, WhatsApp and e-mail unlock after LoadBoot\'s compliance check'),
        h('div', { class: 'cp-row-s', style: 'margin-top:4px' }, 'LoadBoot confirms the dispatcher\'s line, mailbox and agreement first — usually within the first working day of the assignment. Until then, message here: your dispatcher and LoadBoot both see it, and you get an e-mail the moment the line goes live.'),
      ]),
    ]));
  }
  ...
  // ---------- readiness (what the dispatcher needs from the carrier) ----------
  const readiness = card('What your dispatcher needs from you', readyDone + ' of ' + ready.length + ' ready — every missing item is a load you cannot take', h('div', { style: 'display:flex;gap:16px;align-items:flex-start' }, [
    h('div', { class: 'dd-ring', style: '--p:' + readyPct }, h('i', null, readyPct + '%')),
    h('div', { class: 'dd-list', style: 'flex:1;min-width:0' }, ready.map((r) => h('div', { class: 'dd-row' }, [
      h('div', { class: 'dd-check' + (r.done ? ' on' : '') }, icon(r.done ? 'check' : 'dot', 13)),
      h('div', { class: 'l' }, [h('b', null, r.label), h('span', null, r.why || '')]),
      r.done ? null : h('a', { class: 'fix', href: '#' + (r.tab || 'account'), onClick: (e) => { e.preventDefault(); nav(r.tab || 'account'); } }, 'Fix →'),
    ]))),
  ]));
  ...
  if (a && a.sop && Object.keys(a.sop).length) {
    const s = a.sop; const rows = [];
    if (s.min_rate) rows.push(['Rate floor', '$' + s.min_rate + '/mi' + (s.min_rate_note ? ' · ' + s.min_rate_note : '')]);
    if (s.home_time) rows.push(['Home time', s.home_time]);
    if (s.truck) rows.push(['Trucks covered', s.truck]);
    if (s.scope || s.scope_basis) rows.push(['Scope', s.scope || s.scope_basis]);
    rows.push(['Before the driver moves', 'RC goes to LoadBoot first' + (s.driver_moves_only_after_approval === false ? '' : ' · driver moves only after approval')]);
    sop = card('Rules your dispatcher works to', 'Set with LoadBoot at assignment — change them in Account → Dispatch', h('div', { class: 'dd-list' }, rows.map((r) => h('div', { class: 'dd-row' }, [h('div', { class: 'dd-check on' }, icon('check', 13)), h('div', { class: 'l' }, [h('b', null, r[0]), h('span', null, r[1])])]))));
  }
  ...
  if (a) {
    ...
    const paint = async () => {
      try {
        ...
        mount(box, ms.length ? ms.map((m) => h('div', { class: 'dd-msg ' + (m.mine ? 'me' : m.role === 'system' ? 'sys' : 'them') }, [
          m.role !== 'system' ? h('div', { class: 'who' }, (m.mine ? 'you' : m.role === 'staff' ? 'LoadBoot' : (m.by || dp.first_name || m.role)) + ' · ' + when(m.at)) : null, m.body])) : h('div', { class: 'cp-muted' }, 'No messages yet. Say hello — pickup details, availability and rate questions all live here.'));
    ...
    thread = card('Shared thread', 'You · ' + (dp.first_name || 'your dispatcher') + ' · LoadBoot', h('div', null, [
      box, inp, err,
  ...
  const how = card('How the LoadBoot dispatcher program works', 'Plain answers to the four questions every carrier asks', h('div', null, HOW.map((r, i) => h('details', { class: 'dd-faq', open: i === 0 ? '' : null }, [h('summary', null, [h('span', { class: 'ic' }, icon(r[0], 15)), r[1]]), h('p', null, r[2])]))));

  mount(host, h('div', { class: 'dd-wrap' }, [
    hero,
    contact,
    stats,
    a ? h('div', { class: 'dd-grid2' }, [thread, h('div', { style: 'display:grid;gap:14px;align-content:start' }, [sop, readiness].filter(Boolean))]) : readiness,
    callLog,
