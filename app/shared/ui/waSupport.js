...
const CSS = '#lb-wa-fab{position:relative;display:inline-flex;align-items:center;justify-content:center;width:38px;height:38px;border-radius:10px;background:rgba(37,211,102,.12);border:1px solid rgba(37,211,102,.35);color:#25D366;text-decoration:none;flex:none;margin-right:8px;transition:background .15s,color .15s}'
  ...
  // header tooltip: a named pill under the button so the carrier knows what each icon is (chat + WhatsApp)
  + '.cp-top-right [data-lb-tip]{position:relative}'
  + '.cp-top-right [data-lb-tip]::after{content:attr(data-lb-tip);position:absolute;top:calc(100% + 8px);right:0;white-space:nowrap;padding:6px 10px;border-radius:8px;background:#0b1220;color:#eaf1fb;font:700 .72rem/1.2 Inter,system-ui,sans-serif;border:1px solid rgba(255,255,255,.12);box-shadow:0 10px 24px -10px rgba(0,0,0,.7);opacity:0;transform:translateY(-4px);pointer-events:none;transition:opacity .15s,transform .15s;z-index:60}'
  + '.cp-top-right [data-lb-tip]:hover::after,.cp-top-right [data-lb-tip]:focus-visible::after{opacity:1;transform:none}';
...
function build(wa) {
  ...
  a.target = '_blank'; a.rel = 'noopener'; a.setAttribute('data-lb-tip', 'WhatsApp · LoadBoot support' + (wa.display ? ' · ' + wa.display : '')); a.setAttribute('aria-label', 'WhatsApp LoadBoot support');
  a.innerHTML = ICON;
  return a;
}
