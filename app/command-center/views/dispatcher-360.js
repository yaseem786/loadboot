...
function nextActions(dd, test, k, bookings, comm, go, act) {
  ...
  (dd.assignments || []).filter((a) => a.status === 'active').forEach((a) => {
    ...
    if (!a.carrier_ack_at) push(9, 'mail', 'g', (a.carrier || 'Carrier') + ' has not opened the intro yet', (a.carrier_notified_at ? 'Intro e-mail sent ' + dShort(a.carrier_notified_at) + ' — no "Got it" tap yet.' : 'Intro e-mail never sent.') + ' The assignment is active either way; a quick call helps.', 'Open carrier', () => go('carriers'));
  });
