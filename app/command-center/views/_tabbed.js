...
// UX audit CC1 (24 Sep 2026): this strip used to be `.cc-tabbar/.cc-tab` — the same class names the
// shell gives its fixed phone bottom bar (command-center.css ≤780px). On a phone every tabbed screen
// grew a second fixed bar and scrolled sideways (Brokers & shippers 809px wide). Own names now.
const STYLE_ID = 'cc-tabbed-style';
function ensureStyle() {
  if (document.getElementById(STYLE_ID)) return;
  const s = document.createElement('style'); s.id = STYLE_ID;
  s.textContent = [
    '.cc-vtabs{display:flex;gap:2px;border-bottom:1px solid var(--lb-border,#e6edf5);margin:0 0 18px;overflow-x:auto;scrollbar-width:none;-webkit-overflow-scrolling:touch}',
    '.cc-vtabs::-webkit-scrollbar{display:none}',
    '.cc-vtab{border:0;background:none;padding:10px 14px;font:700 .84rem var(--lb-head,Inter,system-ui,sans-serif);color:var(--lb-muted,#64748b);border-bottom:2px solid transparent;margin-bottom:-1px;cursor:pointer;white-space:nowrap;transition:.14s;display:inline-flex;align-items:center;gap:7px}',
    '.cc-vtab:hover{color:var(--lb-navy,#10223B)}',
    '.cc-vtab.active{color:var(--lb-blue,#0883F7);border-bottom-color:var(--lb-blue,#0883F7)}',
    '.cc-vtab .cc-vtab-n{font-size:.68rem;font-weight:800;background:#eef2f7;color:#334155;border-radius:999px;padding:1px 7px;line-height:1.5}',
    '.cc-vtab.active .cc-vtab-n{background:#e0efff;color:var(--lb-blue,#0883F7)}',
    '.cc-tabhost{min-height:200px}',
    '@media (max-width:900px){.cc-vtab{padding:10px 11px;font-size:.8rem}}',
  ].join('\n');
...
export function renderTabbed(host, opts) {
  ...
  const memKey = 'cc-vtab:' + (opts.key || 'x');
  let remembered = null;
  ...
  const btns = {};
  const bar = el('div', { class: 'cc-vtabs', role: 'tablist' }, all.map(t => {
    const b = el('button', { class: 'cc-vtab', role: 'tab', type: 'button', onClick: () => pick(t.id, true) },
      [t.label, t.count != null ? el('span', { class: 'cc-vtab-n' }, String(t.count)) : '']);
    btns[t.id] = b;
    return b;
  }));
