...
function settingsDrawer(onDone) {
  ...
  settings(true).then(S => {
    ...
    // 0410 — standard offer for investors who sign up online (self_onboard)
    const so = S.self_onboard || {}; const sx = so.extra || {};
    const soOn = el('input', { type: 'checkbox', checked: !!so.enabled });
    const soTitle = inp({ value: so.title || '', placeholder: 'e.g. Seed — standard terms' }), soCur = sel(CURRENCIES.map(c => [c, c]), so.currency || 'PKR');
    const soCap = num({ value: so.commitment_cap || '', placeholder: '2000000' }), soRate = num({ value: so.payback_rate_pct != null ? so.payback_rate_pct : 15 }), soShare = num({ value: so.permanent_share_pct != null ? so.permanent_share_pct : 5 });
    const soType = sel([['profit_share', 'Profit share (recommended)'], ['equity', 'Equity']], so.share_type || 'profit_share'), soExit = num({ value: so.exit_participation_pct != null ? so.exit_participation_pct : 5, placeholder: '5' });
    const soStop = sel([['pro_rata', 'Pro-rata (recommended)'], ['keep', 'Keep full %']], so.early_stop_share_mode || 'pro_rata');
    const soProfit = ta({ value: so.profit_definition || '', rows: 2, placeholder: 'Money actually collected in the month minus that month\'s expenses …' });
    const soExitT = ta({ value: so.exit_treatment || '', rows: 2, placeholder: 'If the company is sold, the investor receives X% of the net proceeds …' });
    const soBuy = ta({ value: so.buyout_terms || '', rows: 2, placeholder: 'LoadBoot may buy the investor out at outstanding + 12 months of average payouts …' });
    const soStopT = ta({ value: so.early_stop_terms || '', rows: 2 });
    const soSigner = inp({ value: so.company_signer || DEFAULT_EXTRA.company_signer }), soSignerT = inp({ value: so.company_signer_title || DEFAULT_EXTRA.company_signer_title });
    const soRel = inp({ value: so.relationship_label || 'Signed up online' });
    const xSal = inp({ value: sx.owner_salary_in_expenses || '', placeholder: 'e.g. yes, capped at PKR 150,000/month' }), xTax = inp({ value: sx.tax_treatment || '', placeholder: 'e.g. company taxes are an expense' });
    const xLaw = inp({ value: sx.governing_law || '', placeholder: 'e.g. Wyoming, USA' }), xMed = inp({ value: sx.mediator || '', placeholder: 'e.g. a mutually agreed mediator in Islamabad' });
    const xAcct = inp({ value: sx.separate_account || '' }), xBank = inp({ value: sx.bank_statement_access || '' }), xKey = inp({ value: sx.key_person || '' }), xOther = inp({ value: sx.other_members || '' }), xVisits = inp({ value: sx.visits || '' });
    const xStDay = num({ value: sx.statement_day || DEFAULT_EXTRA.statement_day, step: '1' }), xPay = num({ value: sx.payout_days || DEFAULT_EXTRA.payout_days, step: '1' }), xFlag = num({ value: sx.flag_answer_days || DEFAULT_EXTRA.flag_answer_days, step: '1' }), xReq = num({ value: sx.request_response_days || DEFAULT_EXTRA.request_response_days, step: '1' });
    const soCheck = el('div', { class: 'cc-req-hint' });
    const soExtra = () => ({ owner_salary_in_expenses: xSal.value, tax_treatment: xTax.value, governing_law: xLaw.value, mediator: xMed.value, separate_account: xAcct.value, bank_statement_access: xBank.value, key_person: xKey.value, other_members: xOther.value, visits: xVisits.value, statement_day: xStDay.value, payout_days: xPay.value, flag_answer_days: xFlag.value, request_response_days: xReq.value, company_signer: soSigner.value, company_signer_title: soSignerT.value });
    const soValue = () => ({ enabled: soOn.checked, title: soTitle.value, currency: soCur.value, commitment_cap: Number(soCap.value) || null, payback_rate_pct: Number(soRate.value) || 0, permanent_share_pct: Number(soShare.value) || 0, share_type: soType.value, exit_participation_pct: soExit.value === '' ? null : Number(soExit.value), early_stop_share_mode: soStop.value, profit_definition: soProfit.value, exit_treatment: soExitT.value, buyout_terms: soBuy.value, early_stop_terms: soStopT.value, company_signer: soSigner.value, company_signer_title: soSignerT.value, relationship_label: soRel.value, extra: soExtra() });
    const soPreviewMd = () => { const v = soValue(); return buildAgreement({ title: v.title || 'Investment agreement', currency: v.currency, commitment_cap: v.commitment_cap || 0, payback_rate_pct: v.payback_rate_pct, permanent_share_pct: v.permanent_share_pct, payback_basis: 'actual_funded', share_type: v.share_type, exit_participation_pct: v.exit_participation_pct, early_stop_share_mode: v.early_stop_share_mode, loss_carry_forward: false, profit_definition: v.profit_definition, exit_treatment: v.exit_treatment, early_stop_terms: v.early_stop_terms, buyout_terms: v.buyout_terms, status: 'active', position: { funded: 0, commitment_cap: v.commitment_cap || 0 } }, { name: 'Investor name' }, Object.assign({}, DEFAULT_EXTRA, soExtra()), 'en'); };
    const soPaint = () => { const md = soPreviewMd(); const n = (md.match(/class="ph"/g) || []).length; mount(soCheck, n ? el('span', { style: 'color:#b45309;font-weight:700' }, n + ' blank(s) still in the agreement text — investors will see them and will NOT be able to sign until every blank is filled.') : el('span', { style: 'color:#15803d;font-weight:700' }, 'Agreement text complete — investors can sign.')); };
    [soTitle, soCap, soRate, soShare, soType, soExit, soStop, soProfit, soExitT, soBuy, soStopT, soSigner, soSignerT, xSal, xTax, xLaw, xMed, xAcct, xBank, xKey, xOther, xVisits].forEach(x => x.addEventListener('input', soPaint)); soPaint();
    const soPreviewBtn = el('button', { type: 'button', class: 'lb-btn lb-btn-sm', onClick: () => { const w = window.open('', '_blank'); if (!w) return; w.document.write('<title>Standard offer — preview</title><style>body{font:15px/1.6 system-ui;max-width:820px;margin:32px auto;padding:0 20px}.ph{background:#fef3c7;padding:0 4px}</style>' + mdToHtml(soPreviewMd())); w.document.close(); } }, 'Preview the agreement');
    mount(body, [
      card('Standard offer — investors who sign up online', 'When ON: anyone who creates an account at /app/investor/#signup and is not already in your list gets an investor record + an agreement on these terms, with the agreement text pre-signed by the signer below. They read it in any language and sign; you see them here immediately. When OFF: new sign-ups see "not linked yet" until you add them.',
        el('div', { class: 'cc-inv-set' }, [
          el('label', { style: 'display:flex;gap:8px;align-items:center;font-weight:700;margin:0 0 10px' }, [soOn, 'Enabled — online sign-up creates the agreement automatically']),
          f('Agreement title', soTitle), f('Currency', soCur), f('Commitment (maximum they may put in)', soCap, 'A ceiling, not a debt — same rule as every agreement.'),
          f('Recovery % of profit (until their money is back)', soRate), f('Permanent % of profit (forever)', soShare), f('Share type', soType), f('% of sale proceeds if the company is sold', soExit, 'Blank = none.'),
          f('If they stop funding early', soStop), f('Profit definition', soProfit), f('If the company is sold', soExitT), f('Buyout terms', soBuy), f('Early-stop terms (optional)', soStopT),
          el('h5', { style: 'margin:14px 0 6px' }, 'Wording the agreement needs (these appear as blanks until filled)'),
          f('Owner\'s own salary', xSal), f('Tax treatment', xTax), f('Governing law', xLaw), f('Mediator', xMed), f('Separate bank account for investor money', xAcct, 'e.g. "yes — a dedicated account" or "no — the main operating account, statements shared monthly"'), f('Bank statement access', xBank), f('Key person clause', xKey), f('Other members / co-founders', xOther), f('Office visits', xVisits),
          el('h5', { style: 'margin:14px 0 6px' }, 'Timings'), f('Statement day of month', xStDay), f('Payout within (days)', xPay), f('Answer questions within (days)', xFlag), f('Respond to requests within (days)', xReq),
          el('h5', { style: 'margin:14px 0 6px' }, 'Pre-signature on behalf of LoadBoot LLC'), f('Signer', soSigner), f('Title', soSignerT), f('Relationship label shown in your list', soRel),
          soCheck, el('div', { style: 'margin-top:8px' }, soPreviewBtn),
        ]),
        () => { const v = soValue(); const md = soPreviewMd(); if (v.enabled && hasPlaceholders(md)) throw new Error('Fill every blank before enabling — the investor could not sign an agreement with blanks in it.'); if (v.enabled && !(v.commitment_cap > 0)) throw new Error('Commitment is required.'); return ccInvSettingsSet('self_onboard', v); }),
