...
import { ccAgentsList, ccAgent360, ccAgentDecide, ccAgentMsgs, ccAgentMsgSend, ccAgentNotifySend, referralPayoutDecide, referralPayoutQueue } from '../../shared/api.js';
...
export function renderAgents(host) {
  ...
  async function open360(x) {
    ...
    const docBtn = (label, path) => path ? el('button', { class: 'lb-btn lb-btn-sm lb-btn-secondary', onClick: async (ev) => { const b = ev.currentTarget; const w = b.textContent; b.textContent = '…';
      try { const u = await signedDocumentUrl(path, 600); window.open(u, '_blank', 'noopener'); } catch (e) { alert(humanizeError(e)); } b.textContent = w; } }, label) : el('span', { class: 'cc-pill cc-pill-red' }, label + ' missing');
    ...
    mount(body, el('div', null, [
      ...
      el('div', { class: 'cc-grid-2' }, [
        card([el('h4', { class: 'cc-card-title' }, '👤 Application — everything submitted'),
          ...
          el('div', { style: 'display:flex;gap:8px;margin-top:10px;flex-wrap:wrap' }, [docBtn('🪪 View ID', pd.id_doc), docBtn('🏦 View bank proof', pd.bank_doc)]),
        ...
        card([el('h4', { class: 'cc-card-title' }, '💰 Earnings & payouts'),
          kv('Clearing', money(e9.accrued || 0)), kv('Payable', money(e9.payable || 0)), kv('Paid out', money(e9.paid || 0)),
